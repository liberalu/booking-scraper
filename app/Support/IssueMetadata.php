...
final class IssueMetadata
{
    public const array SEVERITY = [
        ...
        'discover_backend_5xx' => 'warning',
        'discover_empty_first_page' => 'critical',
        'discover_fetch_failed' => 'warning',
        'discover_zero_yield' => 'critical',
        'duplicate_sitemap_url' => 'info',
    ...
    public const array DESCRIPTIONS = [
        ...
        'discover_backend_5xx' => 'A GraphQL discovery page answered 5xx. The range is re-requested in smaller pages; a page that fails again at the smallest size is left for the next run.',
        'discover_empty_first_page' => 'The first category or listing page parsed to zero products. The shop changed its markup, the strategy URL is wrong, or the request was blocked.',
        ...
        'discover_zero_yield' => 'A discover run found no URLs for a shop that already had some. The sitemap or category strategy is broken, not the shop empty.',
        'duplicate_sitemap_url' => 'A sitemap listed the same URL more than once. Harmless, each URL is recorded once; a growing count means the shop duplicates entries across child sitemaps.',
        ...
        'invalid_url' => 'A scraped item carried a URL that is not http(s). The item was dropped; the parser is building URLs from a broken href or base.',
        ...
        'redirect_to_homepage' => 'A product URL redirected to the shop\'s homepage or a top-level page. The product was removed or the slug changed; the URL should be reclassified rather than rescanned.',
        ...
        'url_pattern_filtered' => 'Discovered URLs were excluded by the shop\'s url_include_pattern. A large count on a shop that used to yield means the pattern or the shop\'s URL scheme changed.',
