...
| ~~**EE-2 — Investigate humanitas `orphan_no_url` (8)**~~ | ~~Root cause: non_product discovered_urls for these 8 had shop_book_id=NULL for 7 runs (discovered_url existed but wasn't linked), blocking both orphan detection and non_product_active auto-heal. Today's re-link step set shop_book_id correctly. Triggered a validate run → non_product_active auto-heal fired → all 8 deactivated 2026-05-22. orphan_no_url issues transitioned acknowledged→resolved.~~ | ~~30 min~~ | ✅ 2026-05-22 |
...
| ~~**EE-4 — Auto-acknowledge for confirmed shop-side bugs**~~ | ~~Added `"initial_state": "acknowledged"` to `slug_diacritic_loss` emission in validate.py. Added `initial_state` support to `upsert_validation_issues` (`_make_values` + ON CONFLICT case). Re-detected resolved issues now come back at the requested initial_state, not hardcoded `"new"`. Two lifecycle integration tests added.~~ | ~~1 hr~~ | ✅ 2026-05-22 |
...
| ~~**EE-6 — Add integration tests for the `is_active=true` filter**~~ | ~~Added `test_deactivated_shop_book_does_not_fire_isbn_duplicate` to `test_validate_service.py`. Deactivated shop_book sharing an ISBN with an active one → no `isbn_duplicate` fires. Guards the `is_active=true` gate on all structural duplicate validators.~~ | ~~1 hr~~ | ✅ 2026-05-22 |
...
| ~~**EE-8 — Update data-quality spec's Rollout-Order**~~ | ~~Rule 7 + Rule 9 marked ✅ shipped. Priority order updated: Rule 11 → 12 → 15 → 1 → 14 → 8 → 3 … (matches follow-ups doc).~~ | ~~15 min~~ | ✅ 2026-05-22 |
| ~~**EE-9 — Add "Validator filter cheatsheet" to CLAUDE.md**~~ | ~~Added "Validator filter gates (cheatsheet)" section after the issue-types table. Documents `is_active=true` gate (all shop_book validators), `in_stock=true` gate (price-missing), and both-sides requirement for structural duplicates.~~ | ~~30 min~~ | ✅ 2026-05-22 |
