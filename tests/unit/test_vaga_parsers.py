...
def test_parse_product_page_marks_board_game_as_non_book():
    ...
    reasons = data["book_score_reasons"]
    assert any(r["key"] == "game_toy_title" and r["points"] == -3 for r in reasons)
    assert any(r["key"] == "non_book_categories" and r["points"] == -4 for r in reasons)
...
def test_top_level_book_categories_get_positive_score(category):
    ...
    reasons = result["reasons"]
    assert any(r["key"] == "book_categories" and r["points"] == 3 for r in reasons)
