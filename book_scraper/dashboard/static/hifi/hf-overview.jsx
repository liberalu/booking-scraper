      ...
      <HFShell collapsed={collapsed} setCollapsed={setCollapsed} activePage="overview"
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.55fr 1fr', gap: HF.gap, marginBottom: HF.gap }}>
          ...
          <HFCard title="Metadata completeness" sub="loading…">
            <div style={{padding: HF.cardP}}>
              {[0,1,2,3].map(i => (
                <div key={i} style={{ padding: '8px 0', borderBottom: i < 3 ? `1px solid ${HF.borderFaint}` : 'none' }}>
        ...
        <HFCard title="Recent runs" sub="loading…" style={{ marginBottom: HF.gap }}>
              ...
              return (
                <>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12 }}>
                    <span style={{ fontFamily: HF.mono, fontSize: 26, fontWeight: 600, letterSpacing: -0.5, color: HF.ink, fontVariantNumeric:'tabular-nums' }}>{sumLast.toLocaleString()}</span>
                    <span style={{ fontSize: 13, color: HF.ink3 }}>items this week</span>
          ...
          <div style={{ padding: `${HF.cardP}px`, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
            {clusters.map(c => (
              <a key={c.type} href={buildPath('issues')} className="hf-row"
                ...
                <HFDot tone={c.tone}/>
                <span style={{ fontFamily: HF.mono, fontSize: 13, color: HF.ink, flex: 1 }}>{c.type}</span>
            ...
            {shopCards.map((s, i) => {
              ...
              <a key={s.name} href={buildPath('shop-detail', { name: s.name })}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                  ...
                  <span style={{ fontSize: 15, fontWeight: 600, color: HF.ink }}>{s.name}.lt</span>
                  <HFPill tone={sTone==='ok'?'ok':'err'}>{sStatus}</HFPill>
                  <span style={{ flex: 1 }}/>
                  <span style={{ fontSize: 12, color: HF.ink3, fontFamily: HF.mono, fontVariantNumeric:'tabular-nums' }}>last run {s.last_run_ago}</span>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10 }}>
                  {[['Books', s.books], ['Active', s.active], ['Issues', s.issues]].map(([l, v], j) => (
                    <div key={l} style={{ paddingLeft: j === 0 ? 0 : 12, borderLeft: j === 0 ? 'none' : `1px solid ${HF.borderFaint}` }}>
                      <div style={{ fontSize: 11, color: HF.ink4, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: 600 }}>{l}</div>
