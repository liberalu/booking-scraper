function HFShell({
    ...
    const openAvatar = () => {
        const r =
            avatarRef.current && avatarRef.current.getBoundingClientRect();
        if (window.HF_APP && window.HF_APP.openAvatarMenu)
            window.HF_APP.openAvatarMenu(r);
    };
    const openCmdK = () =>
        window.HF_APP && window.HF_APP.openCmdK && window.HF_APP.openCmdK();
    const openNewRun = () =>
        window.HF_APP && window.HF_APP.openNewRun && window.HF_APP.openNewRun();

    const [runningCount, setRunningCount] = React.useState(0);
    React.useEffect(() => {
        const load = () =>
            fetch("/api/runs?status=running&per_page=1")
                .then((r) => r.json())
    ...
    const navGroups = [
        {
            ...
            items: [
                ...
                [
                    ...
                    "Runs",
                    HF_ICONS.runs,
                    runningCount > 0 ? String(runningCount) : null,
                ],
        ...
        { group: "Data", items: [["prices", "Prices", HF_ICONS.prices]] },
    ];
            ...
            >
                ...
                <div
                    ...
                    style={{
                        flex: 1,
                        overflow: "auto",
                        padding: `20px var(--hf-content-x) 48px`,
                        background: "var(--hf-bg)",
                    }}
                >
