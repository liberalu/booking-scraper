function HFRuns({ nav, goto }) {
    ...
    const [phase, setPhase] = React.useState("crawls");
    ...
    React.useEffect(() => {
        ...
        const load = () =>
            fetch("/api/schedule")
                ...
                .then((d) => {
                    if (!cancelled && d) setScheduleItems(d.items || []);
                })
                .catch(() => {});
        load();
    ...
    return (
        <HFShell
            ...
            activePage="runs"
            title="Runs"
            subtitle="Every scrape execution — manual and scheduled. Click a row to open details."
            breadcrumb={
                <>
                    <span>BookScraper</span>
                    <span style={{ color: "var(--hf-ink5)" }}>/</span>
                    <span style={{ color: "var(--hf-ink)", fontWeight: 500 }}>
                        Runs
                    </span>
                </>
            }
            actions={
                <>
                    <HFButton
                        variant="primary"
                        onClick={() =>
                            window.HF_APP && window.HF_APP.openNewRun()
                        }
                    >
        ...
        >
                    ...
                    {scheduleItems.map((s, i) => (
                        ...
                        >
                            ...
                            {s.next_run_in_s != null && (
                                <span title={s.next_run_at || ""}>
                                    next in{" "}
                                    <strong style={{ color: "var(--hf-ink)" }}>
                                        {_fmtSeconds(s.next_run_in_s)}
                                    </strong>
                                </span>
                            )}
                            <span style={{ color: "var(--hf-ink5)" }}>·</span>
                            <span title={s.last_success_at || ""}>
                                last ok:{" "}
                                <strong style={{ color: "var(--hf-ink)" }}>
                                    {_fmtAgoIso(s.last_success_at)}
                                </strong>
                            </span>
                        </div>
                    ))}
            ...
            <HFKpiStrip
                items={[
                    {
                        label: "Running now",
                        value: String(data.kpis.running_now),
                        delta: (
                            <span style={{ color: "var(--hf-ok-ink)" }}>
                                ● live
                            </span>
                        ),
                        onClick: () => {
                    ...
                    {
                        label: "Last 24h",
                        value: String(data.kpis.today_total),
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>
                                {data.kpis.today_ok} ok ·{" "}
                                {data.kpis.today_failed} failed
                            </span>
                        ),
                        onClick: () => {
                            setStatus("all");
                    ...
                    {
                        label: "All-time",
                        value: String(data.kpis.all_time || 0),
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>
                                total runs
                            </span>
            ...
            <HFCard
                style={{ marginBottom: "var(--hf-gap)", overflow: "visible" }}
                padding={12}
            >
                <HFFilterBar
                    right={
                        <>
                            ...
                            >
                                {data.runs.length.toLocaleString()} of{" "}
                ...
                >
                    <HFSearch
                        placeholder="Search by ID, shop, phase…"
                        width={260}
                        value={q}
                        onChange={setQ}
                    />
                ...
                {loading && data.runs.length === 0 ? (
                    ...
                    >
                        ...
                        {!loading && activeCount > 0 && (
                            ...
                            >
                                shop={shop} · phase={phase} · status={status} ·
                                when={when}
                                {q ? ` · q="${q}"` : ""}
                            </div>
                        )}
                        {!loading && activeCount > 0 && (
                            <HFButton size="sm" onClick={clearAll}>
                                Reset filters
                            </HFButton>
                        )}
                    </div>
                    ...
                    <HFTable
                        ...
                        columns={[
                            ...
                            },
                            {
                                ...
                                sortVal: (r) =>
                                    (r.errors || 0) +
                                    (r.validation_issues || 0),
                                cell: (v, r) => {
                                    const s = r.errors || 0,
                                        vi = r.validation_issues || 0;
                                    if (!s && !vi)
                                        return (
                                            <span
                                                style={{
            ...
            {(data.total || 0) > 0 && (
                ...
                >
                    <span>
                        Showing{" "}
                        {((data.page - 1) * data.per_page + 1).toLocaleString()}
                        –
                        {Math.min(
                            data.page * data.per_page,
                            data.total,
                        ).toLocaleString()}{" "}
                        of {data.total.toLocaleString()} match
                        {data.total === 1 ? "" : "es"}
                        {data.kpis.all_time > data.total && (
                            <span style={{ color: "var(--hf-ink4)" }}>
                                {" "}
                                · {data.kpis.all_time.toLocaleString()} total in
                                DB
                            </span>
                        )}
                    </span>
...
function _eventSummary(eventType, payload) {
    ...
    switch (eventType) {
        case "started": {
            ...
            const total =
                p.urls_total != null ? `${_nfmt(p.urls_total)} URLs` : null;
            const mods = [];
            if (p.rescrape) mods.push("full re-scrape");
            if (p.mode === "single_urls" && Array.isArray(p.urls))
                mods.push(
                    `${p.urls.length} ad-hoc URL${p.urls.length === 1 ? "" : "s"}`,
                );
            if (p.urls_skipped)
                mods.push(`${_nfmt(p.urls_skipped)} skipped (already done)`);
            const left = [phase, total].filter(Boolean).join(" · ");
            return mods.length ? `${left} · ${mods.join(" · ")}` : left;
        }
        case "completed": {
            const processed =
                p.urls_processed != null
                    ? `${_nfmt(p.urls_processed)} URLs processed`
                    : "";
            const errors = p.error_count
                ? `${_nfmt(p.error_count)} error${p.error_count === 1 ? "" : "s"}`
                : "no errors";
            return [processed, errors].filter(Boolean).join(" · ");
        }
        case "failed": {
            const reason =
                CLOSE_REASON_LABELS[p.close_reason] || p.close_reason || "";
            const processed =
                p.urls_processed != null
                    ? `${_nfmt(p.urls_processed)} URLs processed`
                    : "";
        ...
        case "retry_failures": {
            ...
            const filters = [];
            if (p.error_reason_filter)
                filters.push(`reason: ${p.error_reason_filter}`);
            if (p.http_status_filter != null)
                filters.push(`HTTP ${p.http_status_filter}`);
        ...
        case "request_retried": {
            const status =
                p.http_status != null
                    ? `HTTP ${p.http_status}`
                    : p.error || "transport error";
            const attempt =
                p.attempt != null ? `attempt ${p.attempt}` : "retry";
            return `${status} · ${attempt}${p.url ? ` · ${p.url}` : ""}`;
        }
        case "rerun":
            return p.previous_status ? `was ${p.previous_status}` : "";
        case "continued":
            return p.pending_count != null
                ? `${_nfmt(p.pending_count)} URL${p.pending_count === 1 ? "" : "s"} still pending`
                : "";
        ...
        case "subdivided": {
            ...
            const into =
                p.sub_count != null && p.sub_size != null
...
function _eventTone(ev) {
    ...
    if (ev.event_type === "failed") {
        ...
        if (reason === "stopped_by_operator" || reason === "shutdown")
            return "warn";
        return "err";
    }
    if (ev.event_type === "paused" || ev.event_type === "stop_requested")
        return "warn";
    return "accent";
}
...
function _fmtEventTime(iso) {
    ...
    try {
        ...
        if (Number.isNaN(d.getTime()))
            return { short: "—", full: "—", age: "", absolute: "" };
...
function HFRunTimelineCard({ events, style }) {
    ...
    const levelFor = (tone) => {
        if (tone === "err")
            return { label: "ERROR", color: "var(--hf-err-ink)" };
        if (tone === "warn")
            return { label: "WARN ", color: "var(--hf-warn-ink)" };
    ...
    return (
        ...
        >
            {visible.length === 0 ? (
                ...
                >
                    {visible.map((ev) => {
                        const meta = RUN_EVENT_META[ev.event_type] || {
                            label: ev.event_type,
                        };
                        const tone = _eventTone(ev);
                        const lvl = levelFor(tone);
                        const summary = _eventSummary(
                            ev.event_type,
                            ev.payload,
                        );
                            ...
                            <div key={ev.id}>
                                ...
                                {summary ? (
                                    <>
                                        {" "}
                                        <span
                                            style={{ color: "var(--hf-ink5)" }}
                                        >
                                            ·
                                        </span>{" "}
                                        <span
                                            style={{ color: "var(--hf-ink3)" }}
                                        >
                                            {summary}
                                        </span>
                                    </>
                ...
                >
                    <button
                        ...
                        style={{
                            ...
                            fontWeight: 500,
                            padding: "4px 10px",
                            borderRadius: "var(--hf-r2)",
                        }}
                    >
                        {expanded
                            ? `Show latest ${LIMIT} only`
                        ...
                        <HFTabs
                            ...
                            tabs={[
                                ...
                                },
                                {
                                    id: "processing",
                                    label: "processing",
                                    count: tabCounts.processing ?? 0,
                                },
                                {
                                    id: "done",
                                    label: "done",
                                    count: tabCounts.done ?? 0,
                                },
                                {
                                    id: "failed",
                                    label: "failed",
                                    count: tabCounts.failed ?? 0,
                                },
                            ]}
                        />
                        {(urlReason ||
                            ...
                            urlHttpIsNull) &&
                            (() => {
                                const reasonLabel = urlReasonIsNull
                                    ? "unknown"
                                    : urlReason;
                                const httpLabel =
                                    urlHttp != null
                                            ...
                                            {(() => {
                                                ...
                                                return (
                                                    <span
                                                        title={
                                                            tiny
                                                                ? "Suspiciously small response on a 2xx — possible anti-bot stub"
                                            ...
                                            {(() => {
                                                ...
                                                return (
                                                    <span
                                                        title={
                                                            capped
                                                                ? `Retry cap reached (${attempts}/${cap})`
                ...
                >
                    ...
                    >
                        ...
                        <HFButton
                            size="sm"
                            variant="ghost"
                            disabled={urlData.page >= urlData.pages}
                            aria-label="Next page"
                            onClick={() =>
                                setUrlPage((p) =>
                                    Math.min(urlData.pages, p + 1),
                                )
                            }
                        >
                            <span
        ...
        >
            <div style={{ padding: 0 }}>
                {failureGroups.map((g, i) => {
                    ...
                    return (
                        <div
                            key={`${g.reason ?? "__null__"}|${g.http ?? "__null__"}`}
                            style={{
                                borderTop:
                                    i === 0
                                        ? "none"
                        ...
                        >
                            ...
                            >
                                ...
                                >
                                    ...
                                    {(g.acked_count ?? 0) > 0 && (
                                        <span
                                            title={`${g.acked_count} acknowledged event${g.acked_count === 1 ? "" : "s"} in this bucket`}
                                            style={{
                                                display: "inline-flex",
                                                alignItems: "center",
                                                gap: 4,
                                    ...
                                    {(g.capped_count ?? 0) > 0 && (
                                        <span
                                            title={`${g.capped_count} item${g.capped_count === 1 ? "" : "s"} have exhausted the retry cap (${g.max_attempts}/${retryCap} attempts) — retrying won't help until the underlying cause is fixed`}
                                            style={{
                                                display: "inline-flex",
                                                alignItems: "center",
                                                gap: 4,
                                        ...
                                        >
                                            🔒 {g.capped_count} capped (max{" "}
                                            {g.max_attempts}/{retryCap})
                                        </span>
                                    )}
                                    {g.recurring_in_runs > 0 && (
                                        <span
                                            title={`This bucket also failed in ${g.recurring_in_runs} of the last 5 prior runs for this shop`}
                                            style={{
                                                display: "inline-flex",
                                                alignItems: "center",
                                                gap: 4,
                            ...
                            {open && (
                                <div
                                    style={{
                                        padding: `4px var(--hf-card-p) 14px`,
                                        paddingLeft:
                                            "calc(var(--hf-card-p) + 24px)",
                                    }}
                                >
                                    <div
                                        style={{
...
function HFRunInFlightCard({
    ...
    return (
        <HFCard
            ...
            action={
                liveHealth ? (
                    <HFPill tone={healthTone}>
                        <HFDot
                            tone={healthTone}
                            pulse={liveHealth === "healthy"}
                            size={6}
                        />{" "}
                        health: {liveHealth}
                    </HFPill>
            ...
            >
                ...
                >
                    {inFlightFirst ? (
                            ...
                            >
                                <div
                                    style={{
                                        ...
                                        animation:
                                            "hfSweep 1.6s ease-in-out infinite",
                                    }}
                                />
                            </div>
                        </>
                        ...
                        >
                            <HFDot tone="accent" pulse size={7} />
                            {tabCounts.processing} URL
                            {tabCounts.processing > 1 ? "s" : ""} processing ·
                            refreshing…
                        </div>
                    ) : (
                        <div
                        ...
                        >
                            {liveData
                                ? "between requests · waiting for next dispatch…"
                                : "loading…"}
                        </div>
                    )}
                </div>
                            ...
                            style={{
                                background:
                                    rateFailed > 0
                                        ? "var(--hf-err-soft)"
                                        : "var(--hf-subtle)",
                                border: `1px solid ${rateFailed > 0 ? "var(--hf-err-border)" : "var(--hf-border)"}`,
                                borderRadius: "var(--hf-r2)",
                                padding: "10px 12px",
                            }}
                        >
        ...
        <HFCard title="Books">
            ...
            {booksData ? (
                <>
                            ...
                            <tbody>
                                {booksData.books.length === 0 ? (
                                    <tr>
                                        <td
                                            colSpan={
                                                booksTab === "updated" ? 4 : 3
                                            }
                                            style={{
                                                padding: `16px var(--hf-card-p)`,
                                                color: "var(--hf-ink3)",
                                                fontSize: 13,
                                            }}
                                        >
                    ...
                    >
                        <span style={{ fontSize: 13, color: "var(--hf-ink3)" }}>
                            {booksData.total.toLocaleString()} books
                        </span>
                        <div
                            style={{
                                display: "flex",
                        ...
                        >
                            ...
                            {[25, 50, 100].map((n) => (
                                <HFButton
                                    ...
                                    size="sm"
                                    variant={
                                        booksPerPage === n ? "accent" : "subtle"
                                    }
                                    onClick={() => setBooksPerPage(n)}
                                >
                                    {n}
                                </HFButton>
                            ))}
                            ...
                            {(() => {
                                const cur = booksData.page,
                                    total = booksData.pages;
                                const btns = [];
                                const push = (n) =>
                                    btns.push(
                                        <HFButton
                            ...
                            <HFButton
                                size="sm"
                                variant="subtle"
                                disabled={booksData.page >= booksData.pages}
                                aria-label="Next page"
                                onClick={() =>
                                    setBooksPage((p) =>
                                        Math.min(booksData.pages, p + 1),
                                    )
                                }
                            >
                                <span
...
function HFRunDetail({ nav, goto, params }) {
    ...
    const [urlPerPage, setUrlPerPage] = React.useState(
        _initialUrlParamsFull.perPage,
    );
    const [urlReason, setUrlReason] = React.useState(_initialUrlParams.reason);
    const [urlReasonIsNull, setUrlReasonIsNull] = React.useState(
        _initialUrlParams.reasonIsNull,
    );
    const [urlHttp, setUrlHttp] = React.useState(_initialUrlParams.http);
    const [urlHttpIsNull, setUrlHttpIsNull] = React.useState(
        _initialUrlParams.httpIsNull,
    );
    ...
    React.useEffect(() => {
        const sp = new URLSearchParams(window.location.search);
        if (tab !== "general") sp.set("tab", tab);
        else sp.delete("tab");
        if (urlStatus !== "all") sp.set("url_status", urlStatus);
        else sp.delete("url_status");
        if (urlPage !== 1) sp.set("url_page", String(urlPage));
        else sp.delete("url_page");
        if (urlSort !== "started") sp.set("url_sort", urlSort);
        else sp.delete("url_sort");
        if (urlOrder !== "desc") sp.set("url_order", urlOrder);
        else sp.delete("url_order");
        if (urlPerPage !== 10) sp.set("url_per_page", String(urlPerPage));
        else sp.delete("url_per_page");
        if (urlReason) sp.set("url_reason", urlReason);
        else sp.delete("url_reason");
        if (urlReasonIsNull) sp.set("url_reason_is_null", "1");
        else sp.delete("url_reason_is_null");
        if (urlHttp != null) sp.set("url_http", String(urlHttp));
        else sp.delete("url_http");
        if (urlHttpIsNull) sp.set("url_http_is_null", "1");
        else sp.delete("url_http_is_null");
    ...
    React.useEffect(() => {
        ...
        setThroughputHistory((prev) => {
            ...
            return next.length > THROUGHPUT_MAX_SAMPLES
                ? next.slice(next.length - THROUGHPUT_MAX_SAMPLES)
                : next;
        });
    }, [liveData]);
    ...
    React.useEffect(() => {
        ...
        const load = () =>
            fetch(`/api/runs/${runId}/urls?${params.toString()}`)
                .then((r) => r.json())
                .then((d) => {
                    if (!cancelled) setUrlData(d);
                })
                .catch(() => {});
        load();
    ...
    const doRetry = React.useCallback(
        (group) => {
            if (!id) return;
            const params = new URLSearchParams();
            if (group) {
                if (group.reason_is_null)
                    params.set("error_reason_is_null", "true");
                else if (group.reason) params.set("error_reason", group.reason);
                if (group.http_is_null)
                    params.set("http_status_is_null", "true");
                else if (group.http != null)
                    params.set("http_status", String(group.http));
            }
            setActionPending(true);
            fetch(`/api/runs/${id}/retry?${params.toString()}`, {
                method: "POST",
            })
                .then((r) =>
                    r.ok
                ...
                .then((d) => {
                    ...
                    toast({
                        tone: "ok",
                        message: group
                            ? "Group queued for retry"
                            : "Failed URLs queued for retry",
                        detail:
                            n != null
                                ? `${n.toLocaleString()} URL${n === 1 ? "" : "s"} reset to pending.`
                                : "",
                    });
                    fetch(`/api/runs/${id}`)
                        .then((r) => (r.ok ? r.json() : null))
                    ...
                    if (urlHttpIsNull) sp.set("http_status_is_null", "true");
                        ...
                        sp.set("http_status", String(urlHttp));
    ...
    const retryRun = React.useCallback(
        (group) => {
            ...
            setConfirmDialog({
                title: group
                    ? `Retry group on run #${id}`
                    : `Retry all failed URLs on run #${id}`,
                body: `${group ? `Group "${label}"` : "All failed URLs"} will be flipped back to pending.${
                    willRespawn
                        ? " The spider will be re-spawned to pick them up."
                        : ""
                }`,
    ...
    const doAck = React.useCallback(
        (group, note) => {
            ...
            setActionPending(true);
            fetch(`/api/runs/${id}/failures/ack?${params.toString()}`, {
                method: "POST",
            })
                .then((r) =>
                    r.ok
    ...
    const ackGroup = React.useCallback(
        (group) => {
            if (!id || !group) return;
            setAckDialog({ group });
        },
    ...
    const doStop = React.useCallback(() => {
        ...
        fetch(`/api/runs/${id}/stop`, { method: "POST" })
            ...
            .then((d) => {
                ...
                toast({
                    tone: "ok",
                    message: `Run #${id} stopping`,
                    detail: "Spider will exit on its next heartbeat tick.",
                });
            })
            .catch((e) =>
                toast({
        ...
        fetch(`/api/runs/${id}/rerun`, { method: "POST" })
            ...
            .then((d) => {
                ...
                toast({
                    ...
                    detail: newId
                        ? `Run #${newId} is queued for the same shop+phase.`
                        : "",
                });
                goto("runs");
            })
    ...
    const doContinue = React.useCallback(() => {
        ...
        fetch(`/api/runs/${id}/continue`, { method: "POST" })
            ...
            .then((_d) => {
                ...
                toast({
                    tone: "ok",
                    message: `Run #${id} continuing`,
                    detail: "Pending URLs picked up on the same run.",
                });
            })
            .catch((e) =>
                toast({
    ...
    const runsHref =
        (window.HF_BUILD_PATH && window.HF_BUILD_PATH("runs")) || "/runs";
            ...
            <HFShell
                ...
                breadcrumb={
                    <>
                        ...
                        <span style={{ color: "var(--hf-ink5)" }}>/</span>
                        <span style={{ fontFamily: "var(--hf-mono)" }}>
                            #{runId}
                        </span>
                    </>
                }
    ...
    const canContinue = runStatus === "failed" && pendingCount > 0;
    const closeReasonTone =
        runStatus === "failed"
            ? "err"
            : runStatus === "completed" &&
                closeReason === "completed_with_errors"
              ? "warn"
              : runStatus === "completed"
                ? "ok"
                : "neutral";

    const isTerminalForLive =
    ...
    const failPct =
        rateTotal > 0 ? Math.round((rateFailed / rateTotal) * 100) : 0;
    const ratePerMin =
        liveRate.window_s > 0
            ? Math.round((rateDone / liveRate.window_s) * 60)
            : 0;
    ...
    const validationIssueCount = (data.issues || []).reduce(
        (s, g) => s + g.count,
        0,
    );
    ...
    const tabAllCount = Object.values(tabCounts).reduce(
        (a, b) => a + (b || 0),
        0,
    );

    const terminalCount = (tabCounts.done ?? 0) + (tabCounts.failed ?? 0);
    const progressNumerator =
        terminalCount > 0 ? terminalCount : (data.urls_processed ?? 0);
    const errorCount =
        (tabCounts.failed ?? 0) > 0
            ? (tabCounts.failed ?? 0)
            : (data.errors ?? 0);
        ...
        <HFShell
            ...
            title={
                ...
                >
                    ...
                    <HFPill tone={runStatusTone[runStatus] || "neutral"}>
                        <HFDot
                            tone={runStatusTone[runStatus] || "neutral"}
                    ...
                    {(runStatus === "running" ||
                        ...
                        <HFPill
                            tone="muted"
                            title="This page is auto-refreshing every 2 seconds"
                        >
                    ...
                    {isTerminal && closeReason && (
                        <HFPill
                            ...
                            title={`close_reason: ${closeReason}`}
                        >
                            <span
                                style={{
                                    fontFamily: "var(--hf-mono)",
            ...
            subtitle={
                ...
                >
                    shop={data.shop} · phase={data.phase} · started{" "}
                    {data.started_ago} · triggered by {data.by}
                </span>
            }
            breadcrumb={
                <>
            ...
            actions={
                <>
                    ...
                    {(runStatus === "running" ||
                        ...
                        <HFButton
                            variant="danger"
                            disabled={actionPending || runStatus === "stopping"}
                            onClick={stopRun}
                        >
                            <span style={{ display: "flex" }}>
                                {HF_ICONS.stop}
        ...
        >
                ...
                items={[
                    ...
                    },
                    {
                        ...
                        delta: (
                            <span
                                style={{ color: "var(--hf-ink3)" }}
                                title="CONCURRENT_REQUESTS_PER_DOMAIN controls max simultaneous fetches"
                            >
                                in flight
                            </span>
                        ),
                    ...
                    {
                        label: "Val. Issues",
                        value: validationIssueCount.toLocaleString(),
                        ...(validationIssueCount > 0
                            ? { href: `/issues?run_id=${id}` }
                            : {}),
                        delta: (
                            <span
                                style={{
            ...
            {runStatus === "paused" && (
                ...
                >
                    ...
                    <span style={{ flex: 1 }}>
                        <strong style={{ color: "var(--hf-warn-ink)" }}>
                            Run paused
                        </strong>
                        <span
                            style={{
                        ...
                        >
                            Spider is idle but alive — heartbeat is still
                            ticking. Resume to continue scraping.
                        </span>
                    </span>
                    <HFButton
                        variant="primary"
            ...
            {tab === "general" && (
                <>
                    ...
                    >
                        ...
                        />
                        {(() => {
                            const ratePerMinFailed =
                                liveRate.window_s > 0
                                    ? Math.round(
                                          (rateFailed / liveRate.window_s) * 60,
                                      )
                                    : 0;
                            const peak = Math.max(
                                0,
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    0
                                                        ? `${throughputHistory[throughputHistory.length - 1]}/min`
                                                        : "—"}
                                                </span>
                                            </span>
                                            <span
            ...
            {tab === "history" && (
                <>
                    <HFRunHistoryCard
                        ...
                        setUrlPerPage={setUrlPerPage}
                        urlReason={urlReason}
                        setUrlReason={setUrlReason}
                        urlReasonIsNull={urlReasonIsNull}
                        setUrlReasonIsNull={setUrlReasonIsNull}
                        urlHttp={urlHttp}
                        setUrlHttp={setUrlHttp}
                        urlHttpIsNull={urlHttpIsNull}
                        setUrlHttpIsNull={setUrlHttpIsNull}
