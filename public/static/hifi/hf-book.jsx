function HFBook({ nav, goto, params }) {
    ...
    if (loading)
        return (
            <div style={{ padding: 40, color: HF.ink3, fontFamily: HF.sans }}>
                Loading…
            </div>
        );
    if (notFound || !data)
    ...
    const primaryIsbn =
        data.isbns?.find((i) => i.type === "isbn13")?.isbn ||
        data.isbns?.[0]?.isbn ||
        "—";
    ...
    const book = {
        ...
        firstMatched: data.first_matched_at
            ? new Date(data.first_matched_at).toLocaleDateString("en-GB", {
                  day: "numeric",
                  month: "short",
                  year: "numeric",
              })
    ...
    const shops = (data.shops || []).map((s) => ({
        ...
        matchedAt: s.first_seen_at
            ? new Date(s.first_seen_at).toLocaleDateString("en-GB", {
                  day: "numeric",
                  month: "short",
                  year: "numeric",
              })
    ...
    const spread =
        hasPrices && lowest != null && highest != null
            ? (highest - lowest).toFixed(2)
            : null;
    const spreadPct =
        hasPrices && lowest != null && highest != null && highest > 0
            ? (((highest - lowest) / highest) * 100).toFixed(1)
            : null;
    const lowestShop = hasPrices
        ? shops.find((s) => s.price === lowest)?.shop
        : null;
    const highestShop = hasPrices
        ? shops.find((s) => s.price === highest)?.shop
        : null;
    ...
    return (
        <HFShell
            ...
            title={
                ...
                >
                    ...
                    {pending > 0 && (
                        <HFPill tone="warn">{pending} pending review</HFPill>
                    )}
                </span>
            }
            subtitle={
                ...
                >
                    <span>
                        by{" "}
                        <span style={{ color: HF.ink2, fontWeight: 500 }}>
                            {book.author}
                        </span>
                        {" · "}
                        <span style={{ fontFamily: HF.mono, color: HF.ink3 }}>
                            {book.isbns.length > 1
                                ? book.isbns.map((i, idx) => (
                                      <span key={i.isbn}>
                                          {idx > 0 && (
                        ...
                        <span style={{ color: HF.ink3 }}>
                            first matched {book.firstMatched}
                        </span>
                    </span>
            ...
            breadcrumb={
                <>
                    <a
                        ...
                        style={{ color: HF.ink3, textDecoration: "none" }}
                    >
                        Catalog
                    </a>
            ...
            actions={
                <>
                        ...
                        onClick={async () => {
                            ...
                            try {
                                ...
                                if (window.HF_APP?.toast)
                                    window.HF_APP.toast({
                                        kind: "err",
                                        text: `Re-scrape failed: ${e.message}`,
                                    });
                                setRescrapingAll(false);
                            }
                        }}
                    >
                        ...
                        Re-scrape all{rescrapingAll ? "…" : ""}
                    </HFButton>
                    <HFButton
                        onClick={() =>
                            window.HF_APP?.toast?.({
                                kind: "info",
                                text: "Add shop listing — dialog coming soon",
                            })
                        }
                    >
                    ...
                    <HFButton
                        ...
                        onClick={() =>
                            window.HF_APP?.toast?.({
                                kind: "info",
                                text: "Edit book — dialog coming soon",
                            })
                        }
                    >
                        Edit book
        ...
        >
            <HFKpiStrip
                items={[
                    ...
                    },
                    {
                        ...
                        delta:
                            spreadPct != null ? (
                                <span style={{ color: HF.warnInk }}>
                                    {spreadPct}% range
                                </span>
                            ) : null,
                        tone: spreadPct != null ? "warn" : undefined,
                    },
                    {
                        label: "Last matched",
                        value: shops[0]?.lastSeen ?? "—",
                        delta: shops[0] ? (
                            <span style={{ color: HF.ink3 }}>
                                {shops[0].shop}
                            </span>
            ...
            {tab === "metadata" && (
                <HFBookMetadata HF={HF} book={book} shops={shops} />
            )}
        </HFShell>
    );
}
...
function HFBookListings({ HF, shops, goto, methodTone, sbTone, lowest }) {
    return (
        <HFCard
            ...
            action={
                <HFButton size="sm">
                    <span style={{ display: "flex" }}>{HF_ICONS.plus}</span> Add
                    match manually
                </HFButton>
            }
            flush
        >
            <HFTable
                onRowClick={(r) =>
                    goto("shop-book-detail", { id: r.shopBookId })
                }
                columns={[
                    {
                        key: "shop",
                    ...
                    },
                    {
                        ...
                        cell: (_, r) => {
                            ...
                            if (Math.abs(d) < 0.01)
                                return (
                                    <span style={{ color: HF.ink4 }}>0.00</span>
                                );
                            return (
                                <span
                                    style={{
                                        color: d < 0 ? HF.okInk : HF.errInk,
                                        fontWeight: 500,
                                    }}
                                >
                                    {d < 0 ? "▼" : "▲"} €
                    ...
                    },
                    {
                        ...
                        w: "0.6fr",
                        cell: (v) => (
                            <HFPill tone={v === "in stock" ? "ok" : "warn"}>
                                {v === "in stock" ? "on" : "out"}
                            </HFPill>
                        ),
                    },
                    ...
                    },
                    {
                        ...
                        sortable: true,
                        cell: (v) => (
                            <span style={{ color: HF.ink3, fontSize: 11.5 }}>
                                {v}
                            </span>
                        ),
                    },
...
function HFBookPrices({ HF, shops, lowest, history }) {
    if (shops.length === 0 || shops.every((s) => s.price == null)) {
        return (
            <HFCard>
                <div
                    style={{ padding: 40, textAlign: "center", color: HF.ink4 }}
                >
    ...
    const allTimeLow =
        allHistoryPrices.length > 0
            ? allHistoryPrices.reduce(
    ...
    return (
        <>
            <HFCard
                ...
                sub="every shop overlaid · click a shop to isolate"
                            ...
                            >
                                <span
                                    style={{
                                        ...
                                        height: 10,
                                        borderRadius: 2,
                                        background: shopColor(i, HF),
                                    }}
                                />
                                <span
                                    style={{ color: HF.ink, fontWeight: 500 }}
                                >
                                    {s.shop}
                                </span>
                                <span
            ...
            >
                <HFCard
                    ...
                    sub="best deal across matched shops"
                >
                    <div
                        style={{
                            padding: HF.cardP,
                    ...
                    >
                        ...
                        >
                            ...
                            <span style={{ color: HF.ink3, fontSize: 13 }}>
                                {shops.find((s) => s.price === lowest)?.shop ??
                                    ""}
                            </span>
                        </div>
                        <span style={{ fontSize: 12, color: HF.ink3 }}>
                            {lowest != null
                                ? `€${(Math.max(...shops.map((s) => s.price || 0)) - lowest).toFixed(2)} cheaper than the most expensive listing.`
                                : ""}
                        </span>
                    </div>
                </HFCard>
                <HFCard
                    title="All-time low"
                    sub="across all shops, since first match"
                >
                    <div
                        style={{
                            padding: HF.cardP,
                ...
                <HFCard
                    title="Average price"
                    sub="current price across all shops"
                >
                    <div
                        style={{
                            padding: HF.cardP,
                    ...
                    >
                        ...
                        <span style={{ fontSize: 12, color: HF.ink3 }}>
                            {currentPrices.length} shop
                            {currentPrices.length === 1 ? "" : "s"} contributing
                        </span>
                    </div>
                </HFCard>
            </div>
        ...
        <div style={{ overflowX: "auto" }} className="hf-scroll">
            <div style={{ minWidth: 200 + order.length * 140 }}>
                <div
                    style={{
                        display: "grid",
                        gridTemplateColumns: `160px 200px repeat(${order.length}, 1fr)`,
                    ...
                    >
                        ...
                        <span
                            style={{
                                ...
                                textOverflow: "ellipsis",
                                whiteSpace: "nowrap",
                                fontFamily: c.mono ? HF.mono : undefined,
                            }}
                        >
                            {c.canonical || "—"}
                        </span>
                                    ...
                                    >
                                        <span
                                            style={{
                                                ...
                                                border: `1px solid ${HF.warnBorder}`,
                                                display: "inline-flex",
                                                alignItems: "center",
...
function HFIdentifiersCard({ HF, book, shops, order }) {
    ...
    const rows = book.isbns.map((ci) => ({
        ...
        matchFn: (v) =>
            v === ci.isbn
                ? "exact"
                : allCanonicalIsbns.has(v)
                  ? "equiv"
                  : "conflict",
        cells: order.reduce((acc, shopName) => {
            acc[shopName] =
                shops.find((s) => s.shop === shopName)?.shopIsbn ?? null;
            return acc;
        }, {}),
    }));
    return (
        <HFCard
            title="Identifiers"
            sub="canonical ISBNs vs. what each shop reports"
            style={{ marginBottom: HF.gap }}
            flush
        >
            <HFFieldGrid
...
function HFContributorsCard({ HF, book, shops, order }) {
    const rows = (book.contributors || []).map((c) => ({
        ...
        cells: order.reduce((acc, shopName) => {
            ...
            acc[shopName] =
                c.role === "Author" && shopObj?.shopAuthor
                    ? shopObj.shopAuthor
                    : null;
            return acc;
        }, {}),
    ...
    return (
        <HFCard
            title="Contributors"
            sub="people credited on this book — author, translator, editor, cover artist, illustrator, producer, narrator"
            style={{ marginBottom: HF.gap }}
            flush
        >
            <HFFieldGrid
    ...
    const fields = [
        {
            field: "ISBN",
            canonical: book.isbn,
            perShop: (s) => s.shopIsbn,
            match: (v) =>
                book.isbns.length > 0
                    ? book.isbns.some((i) => i.isbn === v)
                    : v === book.isbn,
        },
        { field: "Title", canonical: book.title, perShop: (s) => s.shopTitle },
        {
            field: "Author",
            canonical: book.author,
            perShop: (s) => s.shopAuthor,
        },
        {
            field: "Year",
            canonical: book.year != null ? String(book.year) : null,
            perShop: (s) => (s.shopYear != null ? String(s.shopYear) : null),
        },
        {
            field: "Publisher",
            canonical: book.publisher,
            perShop: (s) => s.shopPublisher,
        },
        {
            field: "Format",
            canonical: book.binding,
            perShop: (s) => s.shopFormat,
        },
        { field: "Language", canonical: book.language, perShop: () => null },
        {
            field: "Pages",
            canonical: book.pages != null ? String(book.pages) : null,
            perShop: () => null,
        },
    ];
        ...
        for (const s of shops) {
            ...
            if (raw == null || raw === "") {
                ...
                const matches = row.match
                    ? row.match(v)
                    : row.canonical != null && v === String(row.canonical);
    ...
    return (
        <>
                                        ...
                                        >
                                            <span
                                                style={{
                                                    ...
                                                    height: 14,
                                                    borderRadius: 3,
                                                    background: HF.okSoft,
                                                    border: `1px solid ${HF.okBorder}`,
                                                    display: "inline-flex",
                                                    alignItems: "center",
                                                    justifyContent: "center",
                                                }}
            ...
            <HFCard
                ...
                sub="fields where a shop disagrees with the canonical value"
                flush
            >
                <div style={{ padding: `6px 0` }}>
                    {matrix
                        .flatMap((row) =>
                            order
                                .filter((name) => row.cells[name]?.conflict)
                                .map((name) => ({
                                    field: row.field,
                                    canonical: row.canonical,
                                    name,
                                    value: row.cells[name].v,
                                })),
                        )
                        .map((c, i, arr) => (
                            <div
...
function ConfidenceBar({ v, HF }) {
    ...
    return (
        ...
        >
            ...
            <span
                style={{
                    color: v >= 0.95 ? HF.ink2 : HF.warnInk,
                    fontWeight: v >= 0.95 ? 500 : 600,
                }}
            >
                {v.toFixed(2)}
            </span>
                            ...
                            {p == null ? (
                                ...
                                <div
                                    style={{
                                        ...
                                        width: `${w}%`,
                                        background: isMin
                                            ? `linear-gradient(90deg, ${HF.ok}, #15803d)`
                                            : `linear-gradient(90deg, ${HF.accent}, ${HF.accentHover})`,
                                        borderRadius: 4,
                                    }}
                                />
                            )}
            ...
            {sorted.map((s, i, a) => (
                <div
                    ...
                    style={{
                        ...
                        padding: `10px ${HF.cardP}px 10px 0`,
                        marginLeft: HF.cardP,
                        gap: 14,
                ...
                >
                    ...
                    <div
                        style={{
                            ...
                            flexDirection: "column",
                            gap: 3,
                            paddingBottom: i < a.length - 1 ? 4 : 0,
                        }}
                    >
                        <div
                            style={{
                        ...
                        >
                            ...
                            <HFPill tone={methodTone[s.method]}>
                                {s.method}
                            </HFPill>
                            <span
                                style={{
...
function MatchStrategy({ HF, shops }) {
    ...
    const labels = {
        ...
        "title+author": [
            "Title + author",
            "fuzzy match when ISBN missing or invalid",
        ],
