    ...
    if (loading || !data) {
        return (
            <HFShell
                ...
                breadcrumb={
                    <>
                        <HFBreadcrumbLink page="urls" goto={goto}>
                            URLs
                        </HFBreadcrumbLink>
                        <span>/</span>
                        <span>#{urlId}</span>
    ...
    return (
        <HFShell
            ...
            subtitle={
                ...
                >
                    {data.url_type} · fail_count={data.fail_count} · discovered{" "}
                    {data.discovered_ago || "—"}
                </span>
            }
            breadcrumb={
        ...
        >
            <HFKpiStrip
                items={[
                    {
                        label: "HTTP status",
                        value: httpCode ? String(httpCode) : "—",
                        tone: httpCode
                            ? httpCode >= 400
                                ? "err"
                                : httpCode >= 300
                    ...
                    },
                    {
                        label: "Last checked",
                        value:
                            data.last_checked_ago ||
                            data.last_scraped_ago ||
                            "—",
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>
                                scraped
                    ...
                    {
                        label: "Fail count",
                        value: String(data.fail_count || 0),
                        tone: isFailing ? "err" : "ok",
                        delta: (
                            <span
                                style={{
                                    color: isFailing
                            ...
                            >
                                {isFailing ? "failing" : "ok"}
                            </span>
                        ),
                    },
                    {
                        label: "Checks",
                        value: String(checkHistory.length),
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>
                                recorded
                            </span>
            ...
            >
                <HFCard
                    title="Response code history"
                    sub={`last ${checkHistory.length} checks`}
                >
                                ...
                                >
                                    {[...checkHistory].reverse().map((c, i) => {
                                        ...
                                        return (
                                            <div
                                                key={i}
                                                title={`${c.when}: HTTP ${c.http_status || "?"}`}
                                                style={{
                                                    width: 22,
                                                    height: 22,
                                                    borderRadius: 3,
            ...
            <HFCard
                title="Check history"
                sub={`${checkHistory.length} most recent checks`}
            >
                {checkHistory.length === 0 ? (
                    <HFEmptyState
                        title="No check history"
                        sub="This URL has not been scraped yet."
                        onClear={null}
                    />
                ) : (
                    <HFTable
...
function HFIssueRelated({ issue, shop, currentId, goto }) {
    ...
    return (
        <HFCard
            title={`Other ${issue} issues in ${shop}`}
            sub={`${related.length} more open ${related.length === 1 ? "issue" : "issues"} of this type`}
        >
            <div>
                {related.map((i, idx) => (
                    <div
...
function HFIssueDetail({ nav, goto, params }) {
    ...
    const lifecycle = async (state) => {
        ...
        try {
            const r = await fetch(
                `/api/issues/${numericId}/lifecycle?state=${state}`,
                { method: "PATCH" },
            );
            if (r.ok) {
                ...
                window.HF_APP?.toast({
                    tone: "ok",
                    message: `Issue marked as ${state.replace("_", " ")}`,
                });
            }
        } finally {
            setActing(false);
    ...
    if (loading) {
        return (
            <HFShell
                ...
                breadcrumb={
                    <>
                        <HFBreadcrumbLink page="issues" goto={goto}>
                            Issues
                        </HFBreadcrumbLink>
                        <span>/</span>
                        <span>…</span>
    ...
    if (!data) {
        return (
            ...
            >
                <HFEmptyState
                    title="Issue not found"
                    sub={`No issue with ID ${rawId} exists.`}
                    onClear={() => goto("issues")}
                />
            </HFShell>
        );
        ...
        <HFShell
            ...
            title={
                ...
                >
                    ...
                    <HFPill tone={lcTone[data.lifecycle_state] || "neutral"}>
                        {lcLabel[data.lifecycle_state] || data.lifecycle_state}
                    </HFPill>
                </span>
            }
            subtitle={
            ...
            actions={
                <>
                    {!isKnown ? (
                        <HFButton
                            onClick={() => lifecycle("already_seen")}
                            disabled={acting}
                        >
                            <span style={{ display: "flex" }}>
                                {HF_ICONS.check}
            ...
            <HFKpiStrip
                items={[
                    { label: "Severity", value: data.severity, tone: sevTone },
                    {
                        label: "Lifecycle",
                        value:
                            lcLabel[data.lifecycle_state] ||
                            data.lifecycle_state,
                        tone: lcTone[data.lifecycle_state] || "neutral",
                    },
                    {
                        label: "Detected",
                        value: data.added_ago,
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>
                                from run #{data.scrape_run_id}
                            </span>
                        ),
                    },
                    {
                ...
                >
                    ...
                    >
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            >
                                {data.shop_book_title ||
                                    `Shop book #${data.shop_book_id}`}
                            </div>
                            <div
                                style={{
                                    fontSize: 12,
