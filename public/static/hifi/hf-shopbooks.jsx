function HFChangesPanel({ changes, fmtDate }) {
            ...
            <span style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                <span
                    style={{
                        ...
                        fontSize: 11,
                        lineHeight: 1.55,
                        color: dimmed ? "var(--hf-ink3)" : "var(--hf-ink)",
                        wordBreak: "break-word",
                        whiteSpace: "pre-wrap",
                    }}
                >
                    {isExp ? str : str.slice(0, TRUNC)}
                    {!isExp && isLong ? "…" : ""}
                </span>
                {isLong && (
                    <button
        ...
        return (
            <HFCard title="Field change history" sub="no changes recorded">
                <HFEmptyState
                    ...
                    sub="Changes are tracked whenever a field value differs from the previous scrape."
                    onClear={null}
                />
            </HFCard>
        );
    ...
    return (
        <HFCard
            ...
            sub={`${changes.length} recorded changes`}
        >
            {changes.map((c, i) => (
                <div
                    key={i}
...
function mdToHtml(text) {
    ...
    return text
        ...
        .map((block) => {
            ...
            const first = lines[0];
            if (/^###\s/.test(first))
                return `<h5 style="margin:0 0 4px">${inline(first.replace(/^###\s*/, ""))}</h5>`;
            if (/^##\s/.test(first))
                return `<h4 style="margin:0 0 6px">${inline(first.replace(/^##\s*/, ""))}</h4>`;
            if (/^#\s/.test(first))
                return `<h3 style="margin:0 0 8px">${inline(first.replace(/^#\s*/, ""))}</h3>`;
...
function HFShopBooks({ nav, goto }) {
    ...
    const [urlUnreachable, setUrlUnreachable] = React.useState(
        _sp.get("unreachable") === "1",
    );
    const _linkedInit =
        _sp.get("linked") === "not_linked"
            ? "not linked"
            : _sp.get("linked") || "any";
    const [linked, setLinked] = React.useState(_linkedInit);
    const [page, setPage] = React.useState(
        Math.max(1, parseInt(_sp.get("page") || "1", 10) || 1),
    );
    const PER_PAGE = 30;
    ...
    React.useEffect(() => {
        setPage(1);
    }, [q, shop, active, missing, bookType, category, urlUnreachable, linked]);
    ...
    return (
        <HFShell
            {...nav}
            activePage="shop-books"
            title="Shop Books"
            subtitle="Every scraped listing from every shop."
            breadcrumb={
                <>
        ...
        >
            <HFKpiStrip
                items={[
                    ...
                    },
                    {
                        ...
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>
                                {data.kpis.total > 0
                                    ? Math.round(
                                          (data.kpis.active / data.kpis.total) *
                                              100,
                                      )
                                    : 0}
                                %
                    ...
                    {
                        label: "Unreachable URL",
                        value: (data.kpis.unreachable || 0).toLocaleString(),
                        delta: (
                            <span style={{ color: "var(--hf-err-ink)" }}>
                                URL dead
                            </span>
                        ),
                        tone:
                            (data.kpis.unreachable || 0) > 0
                                ? "err"
                                : undefined,
                        onClick: () => {
                            clearAll();
                            setUrlUnreachable(true);
                        },
                    },
                    {
                        label: "Missing ISBN",
                        value: data.kpis.missing_isbn.toLocaleString(),
                        delta: (
                            <span style={{ color: "var(--hf-warn-ink)" }}>
                                {data.kpis.total > 0
                                    ? Math.round(
                                          (data.kpis.missing_isbn /
                                              data.kpis.total) *
                                              100,
                                      )
                                    : 0}
            ...
            >
                ...
                >
                    ...
                    <HFFilter
                        label="Type"
                        value={bookType}
                        options={["all", "book", "non_book", "audio", "ebook"]}
    ...
    if (loading || !data) {
        return (
            <HFShell
                ...
                breadcrumb={
                    <>
                        <HFBreadcrumbLink page="shop-books" goto={goto}>
                            Shop Books
                        </HFBreadcrumbLink>
                        <span>/</span>
                        <span>#{bookId}</span>
            ...
            >
                <div style={{ padding: 40, color: "var(--hf-ink3)" }}>
                    Loading…
                </div>
            </HFShell>
        );
        ...
        <HFShell
            ...
            title={
                ...
                >
                    ...
                    <HFPill tone={data.is_active ? "ok" : "neutral"}>
                        {data.status || "unknown"}
                    </HFPill>
                    {data.type && (
                        <HFPill
                    ...
                    {data.book_id && (
                        <span
                            ...
                            onClick={() =>
                                goto("book-detail", { id: data.book_id })
                            }
                        >
                ...
                <>
                    {data.url && (
                        ...
                        >
                            ...
                            Open on {data.shop}.lt
                        </HFButton>
                    )}
                    <HFButton
                        variant="primary"
                        onClick={rescrape}
                        disabled={rescraping || !data.url}
                    >
                        <span style={{ display: "flex" }}>
                            {HF_ICONS.refresh}
                        </span>{" "}
                        {rescraping ? "Queuing…" : "Re-scrape"}
                    </HFButton>
                </>
            }
        >
            {rescrapeError && (
                ...
                >
                    ...
                    >
                        <span
                            style={{ color: "var(--hf-err)", display: "flex" }}
                        >
                            {HF_ICONS.bang}
                        </span>
                        <div style={{ flex: 1 }}>
                ...
                >
                    ...
                    >
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            >
                                {" "}
                                — failed {data.url_fail_count} time
                                {data.url_fail_count !== 1 ? "s" : ""} in a row.
                                The listing has been marked inactive. It will be
                                retried automatically after 7 days.
                            </span>
                        </div>
                        <HFButton
                            size="sm"
            ...
            <HFKpiStrip
                items={[
                    ...
                    {
                        label: "Price records",
                        value: String(priceHistory.length),
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>
                    ...
                    },
                    {
                        label: "Status",
                        value: data.status || "—",
                        tone: data.is_active ? "ok" : "neutral",
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>
                                {data.is_active
                                    ? "active listing"
                                    : `last seen ${data.updated || "—"}`}
                            </span>
                        ),
                    },
                    {
                    ...
                    <HFTabs
                        ...
                        tabs={[
                            ...
                            },
                            {
                                id: "issues",
                                label: "Issues",
                                count: issuesList.length || undefined,
                            },
                            { id: "classification", label: "Classification" },
                            {
                                id: "urls",
                                label: "URLs",
                                count: data.url_count || undefined,
                            },
                            {
                                id: "runs",
                                label: "Runs",
                                count: data.run_count || undefined,
                            },
            ...
            {tab === "overview" && (
                <>
                    ...
                    >
                        {(() => {
                            const isFlat =
                                priceNums.length > 0 && priceMin === priceMax;
                            return (
                                <HFCard
                                    title="Price history"
                                    sub={
                                        isFlat
                                            ? `Stable at €${priceMin?.toFixed(2)} · ${priceHistory.length} records`
                        ...
                        <HFCard title="Metadata" sub="key fields">
                            <div style={{ padding: `2px 0 6px` }}>
                                {[
                                    ...
                                    [
                                        "First seen",
                                        fmtDate(data.first_seen_at),
                                        true,
                                    ],
                                    [
                                        "Last seen",
                                        fmtDate(data.last_seen_at),
                                        true,
                                    ],
                                ].map(([k, v, ok], i, arr) => (
                                    <div
                    ...
                    >
                        <HFCard
                            title="Categories"
                            sub={`${(data.categories || []).filter((c) => c.trim()).length} assigned`}
                        >
                            {(data.categories || []).filter((c) => c.trim())
                                .length === 0 ? (
                                <div
                                    style={{
                        ...
                        <HFCard title="Description" sub="from shop listing">
                            <div
                                ...
                                dangerouslySetInnerHTML={{
                                    __html: mdToHtml(data.description),
                                }}
                            />
                        </HFCard>
                    )}
            ...
            {tab === "prices" && (
                <>
                    <HFKpiStrip
                        items={[
                            ...
                            },
                            {
                                ...
                                delta: (
                                    <span style={{ color: "var(--hf-ink3)" }}>
                                        {priceChanges.length} price changes
                                    </span>
                                ),
                            },
                        ]}
                                ...
                                <div style={{ padding: "var(--hf-card-p)" }}>
                                    {priceHistory.length === 0 ? (
                                        ...
                                        >
                                            ...
                                            >
                                                Price unchanged across all{" "}
                                                {priceHistory.length} scrapes
                                            </div>
                                            <div
                                                style={{
                                                    fontSize: 12,
                    ...
                    <HFCard
                        title="All price records"
                        sub={`last ${priceHistory.length} scrapes`}
                    >
                        {priceHistory.length === 0 ? (
                            <HFEmptyState
                                title="No prices yet"
                                sub="Run a scrape to collect prices."
                                onClear={null}
                            />
                        ) : (
                            <HFTable
            ...
            {tab === "changes" && (
                <HFChangesPanel changes={changes} fmtDate={fmtDate} />
            )}

            {tab === "issues" &&
                (() => {
                    const active = issuesList.filter(
                        (i) => i.lifecycle_state !== "resolved",
                    );
                    const resolved = issuesList.filter(
                        (i) => i.lifecycle_state === "resolved",
                    );
                    const issueRow = (i) => (
                        <div
                            ...
                            style={{
                                ...
                                display: "grid",
                                gridTemplateColumns:
                                    "1.4fr 0.7fr 1fr 0.65fr 0.85fr",
                                gap: 8,
                                alignItems: "center",
                                cursor: "pointer",
                                opacity:
                                    i.lifecycle_state === "resolved" ? 0.5 : 1,
                        ...
                        >
                            ...
                            <HFPill tone={sevTone[i.severity] || "neutral"}>
                                {i.severity}
                            </HFPill>
                            <HFPill
                                tone={
                                    i.lifecycle_state === "resolved"
                                        ? "ok"
                                        : i.lifecycle_state === "acknowledged"
                                          ? "neutral"
                                          : "warn"
                                }
                            >
                    ...
                    return (
                        <HFCard
                            ...
                            sub={`${active.length} active · ${resolved.length} resolved`}
                        >
                            {issuesList.length === 0 ? (
                                <HFEmptyState
                                    title="No issues"
                                    sub="All fields passed validation."
                                    onClear={null}
                                />
                            ) : (
                                <div>
            ...
            {tab === "urls" && (
                <HFCard
                    title="Source URLs"
                    sub="discovered URLs linked to this book"
                >
                    {data.url ? (
                        <>
                            <div
                        ...
                        <HFEmptyState
                            title="No URLs"
                            sub="No URLs linked to this book."
                            onClear={null}
                        />
                    )}
                </HFCard>
            ...
            {tab === "runs" && (
                <HFCard
                    title="Recent runs"
                    sub="runs where this book was scraped"
                >
                    {runs.length === 0 ? (
                        <HFEmptyState
                            title="No runs"
                            sub="This book has not been scraped yet."
                            onClear={null}
                        />
                    ) : (
                        <HFTable
            ...
            {tab === "classification" &&
                (() => {
                    ...
                    const SIGNAL_META = {
                        book_categories: {
                            label: "In book category",
                            dir: 1,
                            max: 3,
                            desc: 'Title belongs to a known book category label (e.g. "Knygos").',
                        },
                        valid_isbn: {
                            label: "Valid ISBN-13",
                            dir: 1,
                            max: 3,
                            desc: "ISBN-13 passes the standard check-digit algorithm.",
                        },
                        author_present: {
                            label: "Author field present",
                            dir: 1,
                            max: 2,
                            desc: "The author field is non-empty after scraping.",
                        },
                        book_metadata: {
                            label: "Book metadata present",
                            dir: 1,
                            max: 2,
                            desc: "At least one of: pages, cover type, year, translator, narrator, duration, format.",
                        },
                        game_toy_title: {
                            label: "Game / toy keyword in title",
                            dir: -1,
                            max: -3,
                            desc: 'Title contains keywords like "žaidimas", "dėlionė", "konstruktorius", etc.',
                        },
                        non_book_categories: {
                            label: "In non-book category",
                            dir: -1,
                            max: -4,
                            desc: "Category matches a known non-book keyword or label.",
                        },
                        blocked_non_book_category: {
                            label: "Blocked — non-book category",
                            dir: -1,
                            max: 0,
                            desc: "Non-book category blocked classification (no overriding book signal).",
                        },
                        game_toy_blocked: {
                            label: "Blocked — game/toy title",
                            dir: -1,
                            max: 0,
                            desc: "Game/toy title blocked classification (no overriding book signal).",
                        },
                        no_title: {
                            label: "No title",
                            dir: -1,
                            max: 0,
                            desc: "Title field missing or empty — cannot classify.",
                        },
                        book_schema: {
                            label: "Structured book schema",
                            dir: 1,
                            max: 2,
                            desc: "Schema.org Book type detected in page markup.",
                        },
                    };
                        ...
                        return (
                            <HFCard title="Classification" sub="no data yet">
                                <HFEmptyState
                                    ...
                                    sub="Classification runs automatically during the scan phase. Re-scrape the book to trigger it."
                                    onClear={null}
                                />
                            </HFCard>
                        );
                    ...
                    const posPoints = reasons.reduce(
                        (s, r) => (r.points > 0 ? s + r.points : s),
                        0,
                    );
                    const negPoints = reasons.reduce(
                        (s, r) => (r.points < 0 ? s + r.points : s),
                        0,
                    );
                    ...
                    const barPct = Math.min(
                        100,
                        Math.max(0, (posPoints / MAX_POSITIVE) * 100),
                    );
                        ...
                        <>
                            <HFCard style={{ marginBottom: "var(--hf-gap)" }}>
                                ...
                                >
                                    ...
                                    >
                                        <span>
                                            Score contribution (positive signals
                                            only)
                                        </span>
                                        <span
                                            style={{
                            ...
                            <HFCard
                                ...
                                sub={`${reasons.length} signals evaluated`}
                            >
                            ...
                            >
                                ...
                                >
                                    {[
                                        ...
                                        {
                                            k: "Primary signal req.",
                                            v: "book category OR valid ISBN OR (author + metadata)",
                                        },
                                        {
                                            k: "Hard blockers",
                                            v: "non-book category / game title (without book signals) → always non-book",
                                        },
                                        {
                                            k: "Max positive score",
                                            v: `${MAX_POSITIVE} pts (categories +3, ISBN +3, author +2, metadata +2)`,
                                        },
                                    ].map(({ k, v }) => (
                                        <div key={k}>
                                            <div
            ...
            {tab === "details" &&
                (() => {
                    const coreRows = [
                        ...
                        [
                            ...
                            data.review_count != null
                                ? Number(data.review_count).toLocaleString()
                                : "—",
                            data.review_count != null,
                        ],
                        [
                    ...
                    const rowStyle = (i, total) => ({
                        display: "grid",
                        gridTemplateColumns: "130px 1fr 14px",
                        padding: `7px var(--hf-card-p)`,
                        alignItems: "center",
                        borderBottom:
                            i < total - 1
                    ...
                    const coreTotal =
                        coreRows.length + (attrs.length > 0 ? 1 : 0);
                    return (
                        <HFCard
                            title="All fields"
                            sub={`${coreRows.length} structured · ${attrs.length} shop attrs`}
                        >
                            <div style={{ padding: `2px 0 6px` }}>
                ...
                >
                    <div
                        style={{
                            ...
                            lineHeight: 1.65,
                            borderTop: `1px solid ${"var(--hf-border-faint)"}`,
                            borderRadius: `0 0 var(--hf-r3) var(--hf-r3)`,
                            whiteSpace: "pre",
                            overflow: "auto",
                        }}
                    >
