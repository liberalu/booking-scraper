function HFOverview({ nav, goto }) {
    ...
    const matchAll = React.useCallback(async () => {
        ...
        try {
            ...
            const started = results.filter(
                (r) => r.status === "fulfilled" && !r.value.detail,
            ).length;
            if (window.HF_APP && window.HF_APP.toast) {
                window.HF_APP.toast({
                    tone: "ok",
                    message: `Match queued for ${started} shop${started !== 1 ? "s" : ""}`,
                });
            }
            goto("runs");
        } catch (e) {
    ...
    const kpis = [
        {
            label: "Shop books",
            value: stats.total_shop_books.toLocaleString(),
            delta: <span style={{ color: "var(--hf-ink3)" }}>total</span>,
            href: buildPath("shop-books"),
            onClick: () => goto("shop-books"),
        },
        {
            label: "Active listings",
            value: stats.active_shop_books.toLocaleString(),
            delta: (
                <span style={{ color: "var(--hf-ink3)" }}>
                    {stats.total_shop_books > 0
                        ? Math.round(
                              (stats.active_shop_books /
                                  stats.total_shop_books) *
                                  100,
                          )
                        : 0}
                    % of total
        ...
        {
            label: "With ISBN",
            value: stats.with_isbn.toLocaleString(),
            delta: (
                <span style={{ color: "var(--hf-ink3)" }}>
                    {stats.total_shop_books > 0
                        ? Math.round(
                              (stats.with_isbn / stats.total_shop_books) * 100,
                          )
                        : 0}
                    % coverage
                </span>
        ...
        },
        {
            ...
            delta: <span style={{ color: "var(--hf-ink3)" }}>total</span>,
            tone: "ok",
            href: buildPath("prices"),
            onClick: () => goto("prices"),
        },
        {
            label: "Open issues",
            value: stats.open_issues.toLocaleString(),
            delta: <span style={{ color: "var(--hf-ink3)" }}>open</span>,
            tone: stats.open_issues > 0 ? "warn" : "ok",
            href: buildPath("issues"),
            onClick: () => goto("issues"),
        },
    ];
    const spark = data.activity || [];
    const completeness = data.completeness.map((c) => [
        c.field.charAt(0).toUpperCase() + c.field.slice(1),
        c.pct,
    ]);
    const statusTone = { running: "ok", completed: "neutral", failed: "err" };
    const runs = data.recent_runs;
    const clusters = data.issue_clusters.map((c) => ({
        type: c.issue_type,
        n: c.count,
        tone: c.count > 100 ? "err" : "warn",
    }));
        ...
        >
            ...
            >
                <HFCard
                    ...
                    sub={`items scraped per day · last ${spark.length || 14} days`}
                    action={
                        <a
                            href={buildPath("runs")}
                            className="hf-link"
                        ...
                        >
                            View runs {HF_ICONS.arrow}
                        </a>
                    }
                >
                    <div
                        style={{
                            padding: `14px var(--hf-card-p) var(--hf-card-p)`,
                        }}
                    >
                        {(() => {
                ...
                <HFCard
                    ...
                    sub="field coverage · active listings"
                    action={
                        <a
                            href={buildPath("shop-books")}
                            style={hfLink(HF)}
            ...
            >
                <HFCard
                    ...
                    sub="open validation clusters · click to triage"
                    action={
                        <a
                            href={buildPath("issues")}
                            style={hfLink(HF)}
