function HFIssueDetail({ nav, goto, params }) {
    ...
    const issueId =
        params?.id && /^\d+$/.test(String(params.id)) ? params.id : null;
        ...
        fetch(`/api/issues?${p}`)
            ...
            .then((d) => {
                ...
                const filtered = issueId
                    ? rows.filter((row) => String(row.id) !== String(issueId))
                    : rows;
                setOtherIssues(
    ...
    const book = apiData
        ? apiData.shop_book_title || null
        : params?.book || "Arturas ir Maltazaro kerštas (DVD)";
    const url =
        apiData?.url ||
        params?.url ||
        "https://patogupirkti.lt/knyga/arturas-ir-maltazaro-kerstas-dvd";
    ...
    const DESC_BY_TYPE = {
        ...
        match_isbn_drift:
            "This shop_book is linked to a canonical book whose ISBN(s) do not match the shop_book ISBN. Either the shop_book ISBN was corrupted (re-scrape) or the canonical link is wrong (unlink + re-match).",
        invalid_isbn:
            "ISBN failed check-digit / length validation. Value cannot be reliably matched.",
        non_product_active:
        ...
        discover_fetch_failed:
            "Sitemap or discovery URL returned 4xx or 5xx — likely permanent removal.",
        unmatched_has_isbn:
        ...
        product_url_non_book:
            "A URL classified as a product page resolved to something that is not a book (DVD, stationery, gift card, etc.).",
    };

    const bulkAckWave = async () => {
        if (
            !window.confirm(
                `Acknowledge all ${waveTotal.toLocaleString()} "${type}" issues${shop ? " in " + shop : ""}?`,
            )
        )
            return;
        ...
        if (r.ok) {
            setApiData((prev) =>
                prev ? { ...prev, lifecycle_state: "acknowledged" } : prev,
            );
        ...
        switch (type) {
                ...
                return [
                    {
                        label: "Open shop",
                        primary: true,
                        action: open("shop-detail", { name: shop }),
                        desc: `View shop detail for ${shop}`,
                    },
                    {
                        label: "Re-scrape URL",
                        action: rescrape,
                        desc: "Trigger a single-URL scan for this book",
                    },
                    {
                        label: "Bulk-ack wave",
                        action: bulkAckWave,
                        desc: `Acknowledge all ${waveTotal.toLocaleString()} ${type} issues in this wave`,
                    },
                ];
            case "match_isbn_drift": {
                const unlinkAndRematch = async () => {
                    ...
                    if (!sbId) {
                        window.alert("No shop_book_id on this issue.");
                        return;
                    }
                    if (
                        !window.confirm(
                    ...
                    const r1 = await fetch(
                        `/api/shop-books/${sbId}/unlink-canonical`,
                        { method: "POST" },
                    );
                    if (!r1.ok) {
                        const d = await r1.json().catch(() => ({}));
                        window.alert(
                            "Unlink failed: " + (d.detail || r1.status),
                        );
                        return;
                    }
                    ...
                    if (r2.ok) {
                        window.alert(
                            `Unlinked. Match run started for ${shop}.`,
                        );
                        goto("runs");
                    } else
                        window.alert(
                            "Unlink ok, but match-run failed: " +
                                (d2.detail || r2.status),
                        );
                };
                return [
                    {
                        ...
                        primary: true,
                        action: rescrape,
                        desc: "Re-fetch the live product page — fixes a corrupted shop_book ISBN",
                    },
                    {
                        label: "Unlink & re-match",
                        action: unlinkAndRematch,
                    ...
                    {
                        label: "Open book",
                        action: open("shop-book-detail", {
                            id: apiData?.shop_book_id,
                        }),
                        desc: "Inspect the shop-book record",
                    },
                    {
                        label: "Bulk-ack wave",
                        action: bulkAckWave,
            ...
            case "unmatched_has_isbn":
                return [
                    ...
                    {
                        label: "Re-run matcher",
                        action: async () => {
                            if (
                                !window.confirm(
                                    `Trigger match run for ${shop}?`,
                                )
                            )
                                return;
                            const r = await fetch("/api/runs", {
                            ...
                            const d = await r.json();
                            if (r.ok) {
                                window.alert(`Match run started for ${shop}.`);
                                goto("runs");
                            } else
                                window.alert(
                ...
                return [
                    {
                        ...
                        primary: true,
                        action: open("shop-detail", { name: shop }),
                        desc: "Manage discovery URLs for this shop",
                    },
                    {
                        label: "Remove URL",
                        action: () =>
                            window.alert("URL removal is not yet implemented."),
                        desc: "Stop scraping this URL",
                    },
                ];
                ...
                return [
                    {
                        label: "Open run",
                        primary: true,
                        action: open("run-detail", { id: runId }),
                        desc: `Inspect run #${runId}`,
                    },
                    {
                        label: "Re-run",
                        action: () =>
                            window.alert("Re-run is not yet implemented."),
                        desc: "Re-trigger with the same parameters",
                    },
                ];
                ...
                return [
                    {
                        ...
                        primary: true,
                        action: open("shop-detail", { name: shop }),
                        desc: "Update URL classification rules",
                    },
                    {
                        label: "Mark URL non-book",
                        action: () =>
                            window.alert(
                                "URL skip-list is not yet implemented.",
                            ),
                        desc: "Add to skip list",
                    },
                    {
                        label: "Bulk-ack wave",
                        action: bulkAckWave,
                        desc: `Acknowledge all ${waveTotal.toLocaleString()} similar`,
                    },
                ];
            default:
                return [
    ...
    const trimmedShown =
        trimUrl && trimUrl.length > 48 ? trimUrl.slice(0, 48) + "…" : trimUrl;
    ...
    return (
                ...
                <>
                    {lifecycle === "new" ? (
                        <HFButton
                            onClick={async () => {
                                const r = await fetch(
                                    `/api/issues/${apiData?.id}/lifecycle?state=acknowledged`,
                                    { method: "PATCH" },
                                );
                                if (r.ok)
                                    setApiData((prev) =>
                        ...
                        <HFButton
                            onClick={async () => {
                                const r = await fetch(
                                    `/api/issues/${apiData?.id}/lifecycle?state=new`,
                                    { method: "PATCH" },
                                );
                                if (r.ok)
                                    setApiData((prev) =>
        ...
        >
            ...
            {waveTotal > 1 && (
                ...
                >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                            Likely a single parser regression, not{" "}
                            {waveTotal.toLocaleString()} separate problems. Ack
                            the entire wave in one action — see "Fix this"
                            below.
                        </div>
                    </div>
                    <HFButton
                        size="md"
                        variant="primary"
                        onClick={() => goto("issues", { run: runId, type })}
                    >
                        View wave →
                    </HFButton>
                </div>
            ...
            >
                <HFCard title="What this means" sub={`issue type: ${type}`}>
                    <div
                        style={{
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {matchContext.book_id && (
                                                ...
                                                >
                                                    Book #{matchContext.book_id}
                                                    {matchContext.book_title
                                                        ? ` — "${matchContext.book_title}"`
                                                        : ""}{" "}
                                                    →
                                                </a>
                ...
                <HFCard title="Failure details">
                    <div style={{ padding: `4px 0 8px` }}>
                        {[
                            ...
                            >
                                <span
                                    style={{ color: HF.ink3, fontSize: 11.5 }}
                                >
                                    {row.k}
                                </span>
                                <span
                    ...
                    >
                        ...
                        >
                            {apiData?.shop_book_id ? (
                                ...
                                >
                                    {book || `Book #${apiData.shop_book_id}`}
                                </a>
                            ) : (
                                <span
                                    style={{
            ...
            >
                {otherIssues.length === 0 && (
                    ...
                    >
                        No other issues of this type found.
                    </div>
                )}
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                ISS-{o.id}{" "}
                                <span
                                    style={{ color: HF.ink5, margin: "0 6px" }}
                                >
                                    ·
                                </span>{" "}
                                {o.issue}
                        ...
                        <span style={{ color: HF.ink4, display: "flex" }}>
                            {HF_ICONS.chevron}
                        </span>
                    </div>
                ))}
