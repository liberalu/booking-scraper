function HFUrls({ nav, goto, params: initParams }) {
    ...
    React.useEffect(() => {
        setPage(1);
    }, [q, shop, urlType, isBook, failing, hasBook, sortBy, sortOrder]);
    ...
    return (
        <HFShell
            ...
            activePage="urls"
            title="URLs"
            subtitle="Every URL the scraper visits — seeds, category pages, and product pages."
            breadcrumb={
                <>
            ...
            actions={
                <>
                    ...
                    <HFButton
                        variant="primary"
                        onClick={() =>
                            window.HF_APP && window.HF_APP.openAddURL()
                        }
                    >
        ...
        >
            <HFKpiStrip
                items={[
                    {
                        label: "Total URLs",
                        value: urlStats.total.toLocaleString(),
                        delta: (
                    ...
                    {
                        label: "In catalog",
                        value: urlStats.in_shop_books.toLocaleString(),
                        delta: (
                    ...
                    },
                    {
                        ...
                        value: urlStats.not_in_shop_books.toLocaleString(),
                        delta: (
                    ...
                    {
                        label: "Failing",
                        value: urlStats.failed.toLocaleString(),
                        delta: (
            ...
            >
                ...
                >
                    <HFSearch
                        placeholder="Search URL, book, shop…"
                        width={300}
                        value={q}
                        onChange={setQ}
                    />
                        ...
                        columns={[
                            ...
                            },
                            {
                                ...
                                cell: (v) => {
                                    ...
                                    return (
                                        <HFPill tone="warn">unscanned</HFPill>
                                    );
                                },
                            },
                            {
...
function HFShops({ nav, goto }) {
    ...
    return (
        <HFShell
            ...
            activePage="shops"
            title="Shops"
            subtitle="Each shop is a scrape target with its own parser, schedule, and rate policy."
            breadcrumb={
                <>
                    <span>BookScraper</span>
                    <span style={{ color: "var(--hf-ink5)" }}>/</span>
            ...
            actions={
                <HFButton
                    variant="primary"
                    onClick={() => window.HF_APP && window.HF_APP.openAddShop()}
                >
...
function HFShopDetail({ nav, goto, params }) {
    ...
    if (loading || !data) {
        return (
            <HFShell
                ...
                breadcrumb={
                    <>
                        <HFBreadcrumbLink page="shops" goto={goto}>
                            Shops
                        </HFBreadcrumbLink>
                        <span>/</span>
                        <span>{shopName}</span>
    ...
    React.useEffect(() => {
        if (tab === "urls")
            fetch(
                `/api/urls?shop=${encodeURIComponent(name)}&per_page=8&sort_by=discovered&sort_order=desc`,
            )
                .then((r) => r.json())
                .then(setUrlsData)
                .catch(() => {});
    ...
    return (
        <HFShell
            ...
            actions={
                <>
                    <HFButton onClick={() => setSettingsOpen(true)}>
                        <span style={{ display: "flex" }}>
                            {HF_ICONS.settings}
                        </span>{" "}
                        Settings
                    ...
                    <HFButton
                        onClick={async () => {
                            ...
                            if (res.ok)
                                window.alert(`Validate started for ${name}.`);
                            else
                                window.alert(
        ...
        >
            <HFKpiStrip
                items={[
                    {
                        label: "Books",
                        value: (data.books || 0).toLocaleString(),
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>
                                total
                            </span>
                    ...
                    {
                        label: "Active",
                        value: (data.active || 0).toLocaleString(),
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>
                                {data.books > 0
                                    ? Math.round(
                                          ((data.active || 0) / data.books) *
                                              100,
                                      )
                                    : 0}
                                %
            ...
            {tab === "overview" && (
                ...
                >
                    ...
                    <HFCard
                        ...
                        sub="% of books with this field set"
                    >
                        <div style={{ padding: `6px 0` }}>
                            {data.field_stats && data.field_stats.total > 0 ? (
                                Object.entries(data.field_stats.fields).map(
                                    ([k, f], i, arr) => {
                                        const pct =
                                            data.field_stats.total > 0
                                                ? (f.present /
                                                      data.field_stats.total) *
                                                  100
                                                : 0;
                                        const t =
                                            pct >= 95
            ...
            {tab === "runs" && (
                <HFRunsPanel goto={goto} scope="shop" entity={name} />
            )}

            {tab === "urls" && (
                <HFCard
                    title="URLs for this shop"
                    sub={
                        `${(urlsData.total || 0).toLocaleString()} total · most recent · ` +
                        (
                            <span
                                style={{
                                    cursor: "pointer",
                    ...
                    action={
                        <HFButton size="sm" onClick={() => goto("urls", {})}>
                            View all URLs →
                        </HFButton>
                    }
                >
            ...
            {tab === "books" && (
                <HFCard
                    title="Books from this shop"
                    sub={`${(data.books || 0).toLocaleString()} total`}
                    action={
                        <HFButton
                            size="sm"
