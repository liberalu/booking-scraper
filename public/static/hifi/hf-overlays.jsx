...
const HF_DISCOVER_STRATEGIES = {
    sitemap: {
        label: "Sitemap",
        hint: "Read /sitemap.xml — fastest, only URL discovery",
    },
    categories: {
        label: "Categories",
        hint: "Walk category listing pages — also extracts prices",
    },
    graphql: {
        label: "GraphQL",
        hint: "Magento GraphQL — full metadata (ISBN, year, pages) in one pass",
    },
    lupasearch: {
        label: "LupaSearch",
        hint: "Fast price + stock + new-arrival pass (no ISBN/year/pages)",
    },
    ibiblioteka_api: {
        label: "Ibiblioteka API",
        hint: "Lithuanian National Library — canonical ISBNs, authors, covers (no prices)",
    },
    full_crawl: {
        label: "Full crawl",
        hint: "Follow every internal link — slowest, most thorough",
    },
};
...
function _hfStrategyOptions(keys) {
    const list =
        Array.isArray(keys) && keys.length
            ? keys
            : Object.keys(HF_DISCOVER_STRATEGIES);
    return list
...
function HFModal({
    ...
    React.useEffect(() => {
        ...
        const id = requestAnimationFrame(() => {
            ...
            const first = Array.from(els).find(
    ...
    React.useEffect(() => {
        ...
        const onKey = (e) => {
            ...
            const els = Array.from(
                panel.querySelectorAll(_HF_FOCUSABLE_SEL),
            ...
            >
                <div
                    ...
                    style={{
                        ...
                        boxShadow:
                            "0 24px 48px -12px rgba(16,24,40,.25), 0 0 0 1px rgba(16,24,40,.04)",
                        overflow: "hidden",
                        display: "flex",
...
function HFModalBody({ children, style }) {
    return (
        <div
            className="hf-scroll"
            style={{ padding: "16px 18px", overflow: "auto", ...style }}
        >
            {children}
        </div>
    );
    ...
    return (
        <div style={{ marginBottom: 14 }}>
            ...
            >
                {label}
                {required && (
                    <span style={{ color: "var(--hf-err-ink)" }}>*</span>
                )}
            </div>
            {children}
            {hint && (
...
function HFInput({
    ...
    return (
        <input
            ...
            onBlur={(e) =>
                (e.currentTarget.style.borderColor = "var(--hf-border-strong)")
            }
            {...rest}
        />
    );
                ...
                return (
                    <button
                        ...
                        style={{
                            ...
                            borderRadius: 4,
                            color: sel ? "var(--hf-ink)" : "var(--hf-ink3)",
...
function HFCommandK({ open, onClose, goto }) {
    ...
    return (
        <HFModal open={open} onClose={onClose} width={620} align="top">
            ...
            >
                <span style={{ color: "var(--hf-ink3)", display: "flex" }}>
                    {HF_ICONS.search}
                </span>
                <input
                    autoFocus
            ...
            <div
                className="hf-scroll"
                style={{ maxHeight: 420, overflow: "auto", padding: 6 }}
            >
                {filtered.length === 0 && (
                    <div
                        style={{
        ...
        try {
            const resp = await fetch("/api/runs", {
                ...
                body: JSON.stringify({
                    shop,
                    phase,
                    strategy: phase === "discover" ? strategy : "",
                    mode,
                }),
            });
            if (!resp.ok) {
                ...
                throw new Error(
                    err.detail || `Request failed (${resp.status})`,
                );
            }
            onClose();
            goto("runs");
    ...
    return (
        <HFModal open={open} onClose={onClose} width={540}>
                ...
                {phase === "discover" && (
                    <HFField label="Strategy" hint={_hfStrategyHint(strategy)}>
                        {shopStrategies.length ? (
                            ...
                            >
                                No discover strategies configured for{" "}
                                {shop || "this shop"}.
                            </div>
                        )}
                    </HFField>
            ...
            <HFModalFoot>
                ...
                >
                    {selShop
                        ? `${urlCount.toLocaleString()} URLs available`
                        : "Loading shops…"}
                </div>
...
function HFNewScheduleDialog({ open, onClose }) {
    ...
    return (
        <HFModal open={open} onClose={() => onClose(false)} width={540}>
            <HFModalHead
                title="New schedule"
                sub="Run a shop on a recurring cron"
            ...
            <HFModalBody>
                ...
                <HFField
                    label="Chain to"
                    hint="Spawn this job automatically after the selected job finishes"
                >
                    <HFSelect
                        value={chainToId ? String(chainToId) : ""}
                        onChange={(v) =>
                            setChainToId(v ? parseInt(v, 10) : null)
                        }
                        options={[
...
function HFEditScheduleDialog({ open, job, onClose }) {
    ...
    const chainOptions = allJobs.filter(
        (j) => j.shop === job?.shop && j.id !== job?.id,
    );
...
function HFAddURLDialog({ open, onClose }) {
    ...
    return (
        <HFModal open={open} onClose={onClose} width={540}>
            <HFModalHead
                title="Add URL"
                sub="Enqueue one or more URLs for scraping"
                onClose={onClose}
                icon={HF_ICONS.urls}
            />
            <HFModalBody>
                ...
                {mode === "single" && (
                    <HFField label="URL" required>
                        <HFInput
                            value={url}
                            onChange={setUrl}
                            placeholder="https://vaga.lt/knygos/…"
                            mono
                            autoFocus
                        />
                    </HFField>
                ...
                {mode === "file" && (
                    ...
                    >
                        ...
                        <div
                            style={{ color: "var(--hf-ink2)", fontWeight: 500 }}
                        >
                            Drop CSV here
                        </div>
            ...
            <HFModalFoot>
                <HFButton onClick={onClose}>Cancel</HFButton>
                <HFButton variant="primary" onClick={onClose}>
                    Enqueue
                </HFButton>
            </HFModalFoot>
        </HFModal>
        ...
        <HFModal open={open} onClose={onClose} width={520}>
            ...
            <HFModalBody>
                ...
                >
                    ...
                    <span>
                        After creating, you'll be taken to the shop detail page.
                    </span>
                </div>
            </HFModalBody>
            <HFModalFoot>
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            if (resp.ok) {
                ...
                window.HF_APP?.toast?.({
                    tone: "ok",
                    message: `Book "${data.title}" created`,
                });
                reset();
                onClose(true);
    ...
    return (
        <HFModal open={open} onClose={handleClose} width={520}>
            ...
            <HFModalBody>
                ...
                <HFField
                    label="ISBN"
                    hint="ISBN-10 or ISBN-13 — leave blank if unknown"
                >
                    <HFInput
                        value={isbn}
                        onChange={(v) => {
                    ...
                    >
                        ...
                        {error.type === "collision" && (
                            <button
                                ...
                                onClick={() => {
                                    handleClose();
                                    window.HF_APP?.goto?.("book-detail", {
                                        id: error.existing_book_id,
                                    });
                                }}
                                style={{
                                    background: "none",
                    ...
                    return (
                        ...
                        >
                            <div style={{ flex: 1 }}>
                                ...
                                >
                                    {(s.books || 0).toLocaleString()} books ·
                                    last run {s.last_run_ago || "—"}
                                </div>
                            </div>
                            <span
                                style={{
        ...
        <HFModal open={open} onClose={onClose} width={680}>
            ...
            <div style={{ display: "flex", minHeight: 380 }}>
                <div
                    style={{
                        width: 160,
                        borderRight: `1px solid ${"var(--hf-border-faint)"}`,
                        padding: 8,
                ...
                <div
                    className="hf-scroll"
                    style={{ flex: 1, padding: 18, overflow: "auto" }}
                >
                    {tab === "appearance" && (
                        <div>
                            <div
                                        ...
                                        <button
                                            ...
                                            style={{
                                                ...
                                                background: sel
                                                    ? HF_ACCENTS[a][50]
                                                    : "var(--hf-surface)",
                                                border: `1px solid ${sel ? c : "var(--hf-border-strong)"}`,
                                                borderRadius: 6,
                                                color: sel
                                                    ? HF_ACCENTS[a][700]
                                                    : "var(--hf-ink2)",
                            ...
                            >
                                Controls row height, card padding, and font
                                size.{" "}
                                {density === "ultra" &&
                                    "Ultra is best for power users with 1440p+ displays."}
                            </div>
                        </div>
                    )}
                    {tab === "notifications" && (
                        <div>
                            {[
                                [
                                    "Run failures",
                                    "Any run that exits with errors",
                                    true,
                                ],
                                [
                                    "Parser regression",
                                    "Field success rate drops > 10%",
                                    true,
                                ],
                                [
                                    "Schedule skipped",
                                    "A cron job was skipped or overran",
                                    true,
                                ],
                                [
                                    "Daily digest",
                                    "Summary at 09:00 every morning",
                                    false,
                                ],
                                [
                                    "URL stuck > 24h",
                                    "Any URL in queued state for a day",
                                    false,
                                ],
                            ].map(([l, s, on]) => (
                                <label
                    ...
                    {tab === "api" && (
                        <div>
                            <HFField
                                label="API key"
                                hint="Use for programmatic access to runs, URLs, and books."
                            >
                                <div style={{ display: "flex", gap: 6 }}>
                                    <HFInput
                                        value="bs_live_8f3a…k29Q"
                                        onChange={() => {}}
                                        mono
                                        readOnly
                                        style={{ fontFamily: "var(--hf-mono)" }}
                                    />
                            ...
                            <HFField
                                label="Webhook URL"
                                hint="POST run events to this URL"
                            >
                                <HFInput
                                    value=""
                                    onChange={() => {}}
                                    placeholder="https://hooks.example.com/bookscraper"
                                    mono
                                />
                            </HFField>
                        ...
                        <div>
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    >
                                        admin@bookscraper.local · prod
                                    </div>
                                </div>
                            </div>
                            <HFField label="Email">
                                <HFInput
                                    value="admin@bookscraper.local"
                                    onChange={() => {}}
                                />
                            </HFField>
        ...
        >
            <div
                ...
                style={{
                    ...
                    borderRadius: 8,
                    boxShadow:
                        "0 12px 32px rgba(16,24,40,.18), 0 0 0 1px rgba(16,24,40,.03)",
                    padding: 4,
                    fontFamily: "var(--hf-sans)",
                }}
            >
                    ...
                    if (it.divider)
                        return (
                            <div
                                ...
                                style={{
                                    height: 1,
                                    background: "var(--hf-border-faint)",
                                    margin: "4px 0",
                                }}
                            />
                        );
...
function HFRateSettingsDialog({ open, onClose, shopName }) {
    ...
    function validate() {
        ...
        if (isNaN(d) || d < 0.1 || d > 60)
            return "Download delay must be 0.1–60 s.";
        if (isNaN(c) || c < 1 || c > 16)
            return "Concurrent requests must be 1–16.";
        return null;
    }
        ...
        <HFModal open={open} onClose={onClose} width={440}>
            ...
            <HFModalBody>
                {loadingSettings ? (
                    ...
                    <>
                        <HFField
                            ...
                            hint="Minimum pause between requests. Range: 0.1 – 60 s."
                        >
                            <HFInput
                                type="number"
                                value={delay}
                        ...
                        <HFField
                            label="Concurrent requests per domain"
                            hint="Max in-flight requests at once. Range: 1 – 16."
                        >
                            <HFInput
                                type="number"
                                value={concurrent}
                ...
                {saved && (
                    ...
                    >
                        Saved. Changes take effect on the next crawl.
                    </div>
                )}
            </HFModalBody>
            <HFModalFoot>
...
function HFConfirmDialog({
    ...
    return (
        <HFModal open={open} onClose={busy ? undefined : onCancel} width={460}>
            ...
            <HFModalFoot>
                <HFButton onClick={onCancel} disabled={busy}>
                    {cancelLabel}
                </HFButton>
                <HFButton
    ...
    return (
        <HFModal open={open} onClose={busy ? undefined : onCancel} width={520}>
            ...
            <HFModalBody>
                ...
                >
                    The bucket will stop appearing on the Failures card. You can
                    still find acknowledged groups via{" "}
                    <strong>Show acknowledged</strong>.
                </div>
                <HFField
                    label="Note"
                    hint='Optional. e.g. "vendor outage 2026-04-28", "URL deleted upstream".'
                >
                    <textarea
                        value={note}
...
function HFToastHost() {
    ...
    return (
        ...
        >
            {toasts.map((t) => (
                <HFToast key={t.id} toast={t} onDismiss={onDismiss} />
            ))}
        </div>
    );
}
