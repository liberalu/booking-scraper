const HF_ISSUE_TYPES = [
    ...
    {
        type: "match_isbn_drift",
        sev: "high",
        tone: "warn",
        description:
            "Shop-book is linked to a canonical book whose ISBN(s) do not match. Either the shop ISBN was corrupted (re-scrape) or the canonical link is wrong (unlink + re-match).",
    },
    {
        type: "invalid_isbn",
        sev: "high",
        tone: "warn",
        description:
            "ISBN check digit fails validation or the value is not 10/13 digits.",
    },
    {
        type: "non_product_active",
        sev: "low",
        tone: "neutral",
        description:
            "A URL classified as non-product is still being scraped as if it were a book listing.",
    },
    {
        type: "price_spike",
        sev: "medium",
        tone: "warn",
        description:
            "Price moved by more than the configured threshold in a single run, with no promo marker.",
    },
    {
        ...
        tone: "warn",
        description:
            "Sitemap / discovery URL returned 4xx or 5xx — likely permanent removal.",
    },
    {
        type: "unmatched_has_isbn",
        sev: "low",
        tone: "neutral",
        description:
            "Shop book carries a valid ISBN but did not link to any canonical book.",
    },
    {
        type: "scrape_run_failed",
        sev: "high",
        tone: "err",
        description:
            "A scrape run ended with status=failed before completing its phase.",
    },
    {
        type: "product_url_non_book",
        sev: "low",
        tone: "neutral",
        description:
            "A URL classified as a product page resolved to something that is not a book.",
    },
];
...
const HF_LIFECYCLE_HELP = {
    ...
    acknowledged:
        "Operator has seen it and accepted it as a real problem to work on.",
    snoozed: "Hidden from the New tab until a wake-up date.",
    resolved:
        "Fixed — either verified clean by a follow-up run, or manually closed.",
...
function formatRelativeAge(isoStr) {
    ...
    const secs = Math.max(
        0,
        Math.floor((Date.now() - new Date(isoStr).getTime()) / 1000),
    );
...
function HFIssues({ nav, goto }) {
    ...
    const _initialParams = React.useMemo(() => {
        ...
        return {
            ...
            sev: ["all", "critical", "warning"].includes(sp.get("sev"))
                ? sp.get("sev")
                : "all",
            q: sp.get("q") || "",
            run,
    ...
    const [lifecycleCounts, setLifecycleCounts] = React.useState({
        new: 0,
        acknowledged: 0,
        snoozed: 0,
        resolved: 0,
    ...
    React.useEffect(() => {
        fetch("/api/issues?per_page=1")
            .then((r) => r.json())
            .then((d) => {
                if (d.counts) setLifecycleCounts(d.counts);
            })
            .catch(() => {});
    }, []);
    ...
    React.useEffect(() => {
        ...
        if (runFilter !== "any")
            params.set("run_id", runFilter.replace("run:", ""));
    ...
    React.useEffect(() => {
        setListPage(1);
        setSelected(new Set());
    }, [view, tab, shopFilter, sevFilter, typeFilter, searchQ, runFilter]);
    ...
    const byTypeRows = groupsData
        .filter((row) => {
            if (typeFilter !== "all" && row.issue_type !== typeFilter)
                return false;
            if (sevFilter === "critical") {
                const meta = HF_ISSUE_TYPES.find(
                    (t) => t.type === row.issue_type,
                );
            ...
            if (searchQ && !row.issue_type.includes(searchQ.toLowerCase()))
                return false;
            return true;
        })
        .map((row) => {
            ...
            const ack =
                (row.by_state?.acknowledged ?? row.cnt_acknowledged) || 0;
            const newCount = (row.by_state?.new ?? row.cnt_new) || 0;
            const priority =
                sevRank[meta.sev || "low"] * Math.log10(Math.max(2, newCount));
    ...
    const listRows = (listData.issues || []).map((issue) => {
        const meta = HF_ISSUE_TYPES.find((t) => t.type === issue.issue) || {
            sev: "low",
            tone: "neutral",
        };
        return {
    ...
    const fixActionsFor = (r) => {
        ...
        const rescrapeUrl = async () => {
            if (!r.url || !r.shop)
                return window.alert("No URL or shop available.");
        ...
        const bulkAck = async () => {
            if (
                !window.confirm(
                    `Acknowledge all "${r.type}" issues${r.shop ? " in " + r.shop : ""}?`,
                )
            )
                return;
            ...
            window.alert(
                res.ok
                    ? `Acknowledged ${d.acknowledged} issues.`
        ...
        const runMatcher = async () => {
            ...
            const d = await res.json();
            if (res.ok) {
                window.alert(`Match run started for ${r.shop}.`);
                goto("runs");
            } else window.alert("Failed: " + (d.detail || res.status));
        };
                ...
                return [
                    {
                        label: "Open shop",
                        primary: true,
                        action: open("shop-detail", { name: r.shop }),
                    },
                ...
                return [
                    {
                        ...
                        primary: true,
                        action: r.shopBookId
                            ? open("shop-book-detail", { id: r.shopBookId })
                            : open("issue-detail", { id: r.id }),
                    },
            ...
            case "discover_fetch_failed":
                return [
                    ...
                    },
                    {
                        label: "Remove URL",
                        action: () =>
                            window.alert("URL removal is not yet implemented."),
                    },
                ];
                ...
                return [
                    {
                        ...
                        primary: true,
                        action: r.runRef
                            ? open("run-detail", { id: r.runRef })
                            : () => {},
                    },
                    {
                        label: "Re-run",
                        action: () =>
                            window.alert("Re-run is not yet implemented."),
                    },
                ];
            default:
                return [
    ...
    const bulkRescrapeGroup = async (issueType, shop) => {
        if (!shop)
            return window.alert(
                "Select a shop first (use the Shop filter) before re-scraping.",
            );
        ...
        if (!qRes.ok) {
            window.alert(
                "Failed to collect URLs: " + (qData.detail || qRes.status),
            );
            return;
        }
        if (!qData.count) {
            window.alert("No URLs found for this issue group.");
            return;
        }
        ...
        if (!sRes.ok) {
            window.alert(
                `Found ${qData.count} URL(s), but scan failed to start: ${sData.detail || sRes.status}`,
            );
            return;
        }
        window.alert(
            `Scan started for ${qData.count} URL(s). When it finishes, run a Validate phase to close the resolved issues (click below or trigger from the Runs page).`,
        );
    };
    ...
    const bulkAcknowledge = () => {
        ...
        const types = [
            ...new Set(selectedRows.map((r) => r.type).filter(Boolean)),
        ];
        const shops = [
            ...new Set(selectedRows.map((r) => r.shop).filter(Boolean)),
        ];
        const body = {};
    ...
    const bulkResolve = () => {
        ...
        Promise.all(
            ids.map((id) =>
                fetch(`/api/issues/${id}/lifecycle?state=resolved`, {
                    method: "PATCH",
                }),
            ),
        )
            ...
            actions={
                <>
                    ...
                    <HFButton
                        ...
                        title="Open issue-handling runbook"
                    >
                        ?
                    </HFButton>
                </>
        ...
        >
                    ...
                    <HFTabs
                        ...
                        tabs={[
                            ...
                            },
                            {
                                id: "acknowledged",
                                label: "Acknowledged",
                                count: lifecycleCounts.acknowledged,
                            },
                            {
                                id: "snoozed",
                                label: "Snoozed",
                                count: lifecycleCounts.snoozed,
                            },
                            {
                                id: "resolved",
                                label: "Resolved",
                                count: lifecycleCounts.resolved,
                            },
                            {
                                id: "all",
                                label: "All",
                                count: lifecycleCounts.total,
                            },
                        ]}
                    />
                    <span
                        title={HF_LIFECYCLE_HELP[tab]}
                        style={{
                            fontSize: 11.5,
                            color: HF.ink3,
                            fontStyle: "italic",
            ...
            {view === "list" && selectedCount > 0 ? (
                ...
                >
                    ...
                    >
                        ...
                        >
                            {selectedCount === 1
                                ? "1 issue"
                                : `${selectedCount.toLocaleString()} issues`}{" "}
                            selected
                        </span>
                        <span style={{ flex: 1 }} />
                        <HFButton
                        ...
                        <HFButton size="sm" onClick={() => bulkSnooze(7)}>
                            Snooze 7d…
                        </HFButton>
                        <HFButton size="sm">Assign…</HFButton>
                        <HFButton size="sm" onClick={bulkResolve}>
                            Mark resolved
                        </HFButton>
                        <HFButton
                            size="sm"
                ...
                >
                    ...
                    >
                        <HFSearch
                            placeholder="Search ID, book, URL, detail…"
                            width={260}
                            value={searchQ}
                            onChange={setSearchQ}
                        />
                        ...
                        >
                            <span style={{ fontSize: 12.5, color: HF.ink3 }}>
                                Run
                            </span>
                            <input
                                type="text"
                                ...
                                onChange={(e) =>
                                    setRunFilter(e.target.value || "any")
                                }
                                style={{
                                    width: 80,
                                    height: 30,
            ...
            {false && (
                <>
                    <HFCard flush style={{ marginBottom: HF.gap }}>
                        ...
                        >
                            Waves removed — use filters (Shop + Type + Run) on
                            the List view to get the same grouping.
                        </div>
                        {itemWaves.map((w, i, arr) => {
                            const meta =
                                HF_ISSUE_TYPES.find((t) => t.type === w.type) ||
                                {};
                            ...
                            return (
                                ...
                                >
                                    ...
                                    >
                                        <HFButton size="sm" variant="primary">
                                            Ack wave
                                        </HFButton>
                                        <HFButton
                                            size="sm"
                                            onClick={() => {
                                                setTypeFilter(w.type);
                                                if (w.shop)
                                                    setShopFilter(w.shop);
                                                setRunFilter(String(w.run));
                                                setView("list");
                                            }}
                                        >
                                            View
                            ...
                            >
                                <span
                                    style={{
                                        ...
                                        height: 28,
                                        borderRadius: 6,
                                        background: HF.errSoft,
                                        border: `1px solid ${HF.errBorder}`,
                                        color: HF.errInk,
                                        fontFamily: HF.mono,
                                        fontWeight: 700,
                                    }}
            ...
            {false && (
                <HFCard flush>
                    ...
                    >
                        ...
                        <span style={{ textAlign: "right" }}>Ack</span>
                        <span />
                    </div>
                    {byTypeShopRows.map((r, i, arr) => (
                        ...
                        >
                            ...
                            <span style={{ fontSize: 12.5, color: HF.ink2 }}>
                                {r.shop}
                            </span>
                            <span
                                style={{
                            ...
                            title={
                                tab === "resolved"
                                    ...
                                    : tab === "snoozed"
                                      ? "No snoozed issues"
                                      : tab === "acknowledged"
                                        ? "Nothing acknowledged in this filter"
                                        : "No issues match these filters"
                            }
                            sub={
                                tab === "resolved"
                                    ? "Issues move here automatically when the next clean run passes for the same URL."
                                    : tab === "snoozed"
                                      ? "Snooze an issue to hide it from New until a wake-up date."
                                      : tab === "acknowledged"
                                        ? 'Tick rows in List view and click "Mark acknowledged" to move them here.'
                ...
                return (
                    <React.Fragment key={r.id}>
                        ...
                        >
                            ...
                            <span>
                                <HFPill tone={sevTone[r.sev]}>{r.sev}</HFPill>
                            </span>
                            <span
                                style={{
                                    fontFamily: HF.mono,
