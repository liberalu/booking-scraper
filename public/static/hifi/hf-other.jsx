function HFCron({ nav, goto }) {
    ...
    const filters = useHFFilters(jobs, {
        search: {
            fields: (j) =>
                `${j.name} ${j.cron || ""} ${j.shop} ${j.chain_to_name || ""}`,
        },
        filters: [
        ...
        <HFShell
            ...
            actions={
                <HFButton
                    variant="primary"
                    onClick={() =>
                        window.HF_APP && window.HF_APP.openNewSchedule()
                    }
                >
                    <span style={{ display: "flex" }}>{HF_ICONS.plus}</span> New
                    schedule
                </HFButton>
            }
        >
            ...
            >
                    ...
                    right={
                        <>
                            <span
                                style={{
                                    fontSize: 12,
                                    color: filters.activeCount
                                        ? "var(--hf-accent-ink)"
                                        : "var(--hf-ink4)",
                                    fontFamily: "var(--hf-mono)",
                                    fontVariantNumeric: "tabular-nums",
                                    fontWeight: filters.activeCount ? 500 : 400,
                                }}
                            >
                ...
                >
                    ...
                    <HFFilter
                        label="Shop"
                        value={filters.vals.shop}
                        options={[...shopNames, "—"]}
                        onChange={(v) => filters.setVal("shop", v)}
                    />
                    <HFFilter
                        label="State"
                        value={filters.vals.state}
                        options={["all", "active", "failing", "disabled"]}
                        onChange={(v) => filters.setVal("state", v)}
                    />
                </HFFilterBar>
            </HFCard>
                    ...
                    <HFTable
                        onRowClick={(r) =>
                            goto("schedule-detail", {
                                ...
                                chain_to_id: r.chain_to_id,
                                chain_to_name: r.chain_to_name,
                            })
                        }
                        columns={[
                            {
                                            ...
                                            >
                                                ...
                                                >
                                                    ...
                                                    >
                                                        ...
                                                        />
                                                        <path
                                                            d="M6 6 L4 8 Q2 10 4 12 Q6 14 8 12 L10 10"
                                                            stroke="currentColor"
                                                            strokeWidth="1.6"
                                                            strokeLinecap="round"
                                                        ...
                                                        />
                                                        <path
                                                            d="M10 10 L12 8 Q14 6 12 4 Q10 2 8 4 L6 6"
                                                            stroke="currentColor"
                                                            strokeWidth="1.6"
                                                            strokeLinecap="round"
                                                            strokeLinejoin="round"
...
function HFIssues({ nav, goto }) {
    ...
    const [tab, setTab] = React.useState(() => {
        const t = _sp().get("tab");
        return ["new", "acknowledged", "snoozed", "resolved", "all"].includes(t)
            ? t
            : "new";
    });
    const [page, setPage] = React.useState(() =>
        Math.max(parseInt(_sp().get("page") || "1", 10) || 1, 1),
    );
    const [severity, setSeverity] = React.useState(
        () => _sp().get("severity") || "all",
    );
    const [issueType, setIssueType] = React.useState(
        () => _sp().get("issue_type") || "all",
    );
    ...
    const [urlTypeFilter, setUrlTypeFilter] = React.useState(
        () => _sp().get("url_type") || "all",
    );
    const [bookTypeFilter, setBookTypeFilter] = React.useState(
        () => _sp().get("book_type") || "all",
    );

    const runId =
        parseInt(runIdInput, 10) > 0 ? parseInt(runIdInput, 10) : null;
    ...
    const ISSUE_REFERENCE = [
        {
            key: "isbn_duplicate",
            sev: "critical",
            desc: "Same ISBN, two rows in same shop",
        },
        {
            key: "in_stock_no_price",
            sev: "critical",
            desc: "In-stock book has no price",
        },
        {
            key: "non_product_active",
            sev: "critical",
            desc: "URL marked non-product but shop_book is active",
        },
        {
            key: "unreachable_active",
            sev: "critical",
            desc: "URL is unreachable but shop_book still active",
        },
        {
            key: "slug_title_mismatch",
            sev: "warning",
            desc: "URL slug shares zero tokens with book title",
        },
        {
            key: "active_no_price",
            sev: "warning",
            desc: "Active book has no price at all",
        },
        {
            key: "stale_active",
            sev: "warning",
            desc: "Active but not seen in last 28 days",
        },
        {
            key: "non_book_has_isbn",
            sev: "warning",
            desc: "Type=non_book but has a valid ISBN",
        },
        {
            key: "unmatched_has_isbn",
            sev: "warning",
            desc: "Has ISBN but not matched to canonical book",
        },
        {
            key: "match_isbn_drift",
            sev: "warning",
            desc: "Matched book's ISBN differs from shop_book ISBN",
        },
        {
            key: "book_no_metadata",
            sev: "info",
            desc: "type=book but no ISBN, author, or year",
        },
        {
            key: "no_price_history",
            sev: "info",
            desc: "Active but zero rows in prices table",
        },
        {
            key: "year_out_of_range",
            sev: "info",
            desc: "Year < 1800 or > current year + 2",
        },
        { key: "price_zero", sev: "info", desc: "Price is exactly 0" },
        {
            key: "format_is_dimensions",
            sev: "info",
            desc: "Format field looks like dimensions (e.g. 210×148)",
        },
        {
            key: "book_no_signals",
            sev: "info",
            desc: "type=book but no ISBN, author, year, or format",
        },
        {
            key: "orphan_no_url",
            sev: "info",
            desc: "shop_book has no linked discovered_url row",
        },
        {
            key: "url_aliases",
            sev: "info",
            desc: "Same shop_book linked from multiple URLs",
        },
        {
            key: "product_url_non_book",
            sev: "info",
            desc: "URL type=product but shop_book type=non_book",
        },
        {
            key: "title_author_duplicate",
            sev: "info",
            desc: "Same title+author, two rows in same shop",
        },
    ];
    ...
    React.useEffect(() => {
        ...
        severity !== "all"
            ...
            : sp.delete("severity");
        issueType !== "all"
            ? sp.set("issue_type", issueType)
            : sp.delete("issue_type");
        ...
        urlTypeFilter !== "all"
            ? sp.set("url_type", urlTypeFilter)
            : sp.delete("url_type");
        bookTypeFilter !== "all"
            ? sp.set("book_type", bookTypeFilter)
            : sp.delete("book_type");
    ...
    const allVisibleSelected =
        allIssues.length > 0 && allIssues.every((r) => selected.has(r.id));
    ...
    const HeaderCheck = () => {
        const state = allVisibleSelected
            ? "all"
            : someVisibleSelected
              ? "some"
              : "none";
    ...
    const dimIfKnown = (row, node) =>
        row.known ? <span style={{ opacity: 0.48 }}>{node}</span> : node;
        ...
        <HFShell
            ...
            actions={
                <>
                    ...
                    <HFButton
                        size="sm"
                        variant={showHelp ? "primary" : "subtle"}
                        onClick={() => setShowHelp((h) => !h)}
                        title="Show all issue types and severities"
                    >
                        ?
                    </HFButton>
                </>
        ...
        >
            <HFKpiStrip
                items={[
                    {
                        label: "New",
                        value: String(byTab.new),
                        delta: (
                            <span style={{ color: "var(--hf-err-ink)" }}>
                                new
                            </span>
                        ),
                        tone: byTab.new > 0 ? "err" : "ok",
                    },
                    {
                        label: "Acknowledged",
                        value: String(byTab.acknowledged),
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>
                                acknowledged
                            </span>
            ...
            >
                <HFModalHead
                    title="Issue type reference"
                    sub="21 check types across 5 groups"
                    onClose={() => setShowHelp(false)}
                />
                <HFModalBody>
                    <div
                ...
                >
                    ...
                    >
                        ...
                        <HFFilter
                            label="Shop"
                            value={shopFilter}
                            options={["all", ...shopsList.map((s) => s.name)]}
            ...
            {undoToast && (
                ...
                >
                    <span>
                        {(undoToast.count || 0).toLocaleString()} issues
                        acknowledged
                    </span>
                    <button
                        onClick={() => {
                            ...
                            fetch("/api/issues/bulk-unacknowledge", {
                                ...
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({
                                    issue_type: undoToast.issue_type,
                                    shop: undoToast.shop,
                                }),
                            }).then(() => {
                                setUndoToast(null);
            ...
            <HFCard>
                <div
                    style={{
                        display: "flex",
                        alignItems: "center",
                        padding: "10px var(--hf-card-p) 10px",
                                        ...
                                        style={{
                                            ...
                                            boxShadow:
                                                viewMode === m.id
                                                    ...
                                                    : "none",
                                            transition:
                                                "background 150ms, color 150ms, box-shadow 150ms",
                                        }}
                                    >
                                        <span
                                            style={{
                ...
                {viewMode === "list" ? (
                    allIssues.length === 0 ? (
                        <HFEmptyState
                            title={
                                tab === "acknowledged"
                                    ? "No acknowledged issues yet"
                                    : tab === "snoozed"
                                      ? "No snoozed issues"
                                      : tab === "resolved"
                                        ? "Resolved issues appear here"
                                        : "No issues match these filters"
                            }
                            sub={
                                tab === "acknowledged"
                                    ? 'Tick an issue\u2019s checkbox and click "Mark as known" to acknowledge it as expected.'
                                    : tab === "snoozed"
                                      ? "Snooze an issue to hide it until later."
...
function HFPrices({ nav, goto }) {
    ...
    const allRows = data.changes.map((c) => ({
        ...
        title: c.title,
        prev:
            c.prev_price !== null ? `€${Number(c.prev_price).toFixed(2)}` : "—",
        now: c.new_price !== null ? `€${Number(c.new_price).toFixed(2)}` : "—",
        pct:
            c.prev_price && c.prev_price !== 0
                ? (c.change / c.prev_price) * 100
                : 0,
        absChg: c.change,
        when: c.scraped_ago,
    ...
    const biggestDrop = allRows.reduce(
        (best, r) => (r.pct < (best?.pct ?? 0) ? r : best),
        null,
    );
    ...
    return (
        <HFShell
            ...
            activePage="prices"
            title="Prices"
            subtitle="Price movements across all scraped listings."
            breadcrumb={
                <>
                    <span>BookScraper</span>
                    <span style={{ color: "var(--hf-ink5)" }}>/</span>
                ...
                items={[
                    ...
                    ...(biggestDrop
                        ? [
                              {
                                  label: "Biggest drop",
                                  value: `${Math.round(biggestDrop.pct)}%`,
                                  tone: "err",
                                  delta: (
                                      <span
                                          style={{
                    ...
                    ...(biggestRise
                        ? [
                              {
                                  label: "Biggest rise",
                                  value: `+${Math.round(biggestRise.pct)}%`,
                                  tone: "ok",
                                  delta: (
                                      <span
                                          style={{
            ...
            <HFCard style={{ marginBottom: "var(--hf-gap)" }}>
                ...
                {loading && rows.length === 0 ? (
                    ...
                    <HFEmptyState
                        ...
                        sub={`No price movements in the last ${days} days.`}
                        onClear={null}
                    />
                ) : (
                    <HFTable
