            ...
            <HFTable
                ...
                columns={[
                    ...
                    },
                    {
                        ...
                        w: "0.8fr",
                        sortable: true,
                        cell: (v) => <HFPill tone={statusTone[v]}>{v}</HFPill>,
                    },
                    {
                        key: "_",
                        label: "",
...
function HFPricesPanel() {
    ...
    const history = [
        19.9, 19.9, 18.5, 18.5, 18.5, 19.5, 19.5, 21.0, 19.9, 19.9, 19.9, 19.9,
        18.9, 19.9, 19.9, 19.9, 18.5, 19.9, 19.9, 19.9, 19.9, 18.9, 19.9, 19.9,
    ...
    return (
        <div>
            ...
            <HFCard
                ...
                sub="30 data points · last 30 days"
            ...
            <HFCard
                ...
                sub={`${changes.length} recorded movements`}
            >
                <HFTable
                    columns={[
                        {
...
function HFScheduleDetail({ nav, goto, params }) {
    ...
    const nextKpi = upcoming[0]
        ? {
              value: upcoming[0].when,
              delta: (
                  <span style={{ color: "var(--hf-ink3)" }}>
                      {upcoming[0].at} · {upcoming[0].date}
                  </span>
              ),
          }
        : {
              value: "—",
              delta: <span style={{ color: "var(--hf-ink3)" }}>disabled</span>,
          };
        ...
        <HFShell
            ...
            title={
                ...
                >
                    ...
                    {enabledState ? (
                        <HFPill tone={lastStatus === "fail" ? "err" : "ok"}>
                            {lastStatus === "fail" ? "failing" : "active"}
                        </HFPill>
                    ) : (
            ...
            breadcrumb={
                <>
                    <HFBreadcrumbLink page="cron" goto={goto}>
                        Schedules
                    </HFBreadcrumbLink>
                ...
                <>
                    <HFButton onClick={toggleEnabled} disabled={toggling}>
                        <span style={{ display: "flex" }}>
                            {enabledState ? HF_ICONS.stop : HF_ICONS.play}
                        </span>
        ...
        >
            <HFKpiStrip
                items={[
                    {
                        label: "Next run",
                        value: enabledState ? nextKpi.value : "—",
                        tone: enabledState ? "accent" : undefined,
                        delta: enabledState ? (
                            nextKpi.delta
                        ) : (
                            <span style={{ color: "var(--hf-ink3)" }}>
                    ...
                    },
                    {
                        label: "Last 24h",
                        value:
                            stats.total_24h != null
                                ? `${stats.total_24h} runs`
                                : "—",
                        delta:
                            stats.total_24h != null ? (
                                <span style={{ color: "var(--hf-ok-ink)" }}>
                                    {stats.ok_24h} ok · {stats.fail_24h} failed
                                </span>
                            ) : null,
                    },
                    {
                        ...
                        value:
                            stats.success_rate_30d != null
                                ? `${stats.success_rate_30d}%`
                                : "—",
                        delta: (
                            <span style={{ color: "var(--hf-ink3)" }}>30d</span>
                        ),
                        tone: stats.success_rate_30d != null ? "ok" : undefined,
                    },
                    {
                        label: "Avg duration",
                        value: stats.avg_dur || "—",
            ...
            >
                        ...
                        {last24.length === 0 ? (
                            ...
                            <div style={{ display: "flex", gap: 3 }}>
                                {last24.map((s, i) => (
                                    <div
                                        key={i}
                                        title={`run ${last24.length - i} ago: ${s}`}
                                        style={{
                                            flex: 1,
                                            height: 22,
                                            borderRadius: 2,
                ...
                <HFCard
                    title="Upcoming runs"
                    sub={enabledState ? "next 5 scheduled" : "job is disabled"}
                >
                    {!enabledState ? (
                        <div
                            style={{
                        ...
                        >
                            ...
                            >
                                No upcoming runs — job is disabled.
                            </div>
                        </div>
            ...
            {tab === "logs" && (
                <HFCard
                    title="Latest logs"
                    sub="log streaming not yet available"
                >
                    <div style={{ padding: "32px 16px", textAlign: "center" }}>
                        ...
                        >
                            Log streaming not yet available
                        </div>
                        <div
                            style={{
                                fontSize: 12,
                        ...
                        >
                            Run events and detailed logs will appear here in a
                            future release.
                        </div>
                    </div>
                </HFCard>
            ...
            {editOpen && (
                <HFEditScheduleDialog
                    ...
                    job={{
                        ...
                        strategy: detail?.strategy,
                        cron,
                        chain_to_id: detail?.chain_to_id ?? null,
                    }}
                    onClose={(saved) => {
                        setEditOpen(false);
                        if (saved) reload();
            ...
            >
                ...
                <HFModalBody>
                    ...
                    deleteState.dependents.length > 0 ? (
                        <>
                            <div
                                style={{
                                    fontSize: 13,
                            ...
                            >
                                Cannot delete — these schedules chain to this
                                one. Unlink each one first (open it, click Edit,
                                clear the chain), then come back to delete.
                            </div>
                            <ul
                                style={{
                                    margin: 0,
                        ...
                        <div
                            style={{ fontSize: 13, color: "var(--hf-err-ink)" }}
                        >
                            {deleteState.error}
                        </div>
                    ) : (
                        <div style={{ fontSize: 13, color: "var(--hf-ink2)" }}>
                            Delete schedule <strong>{name}</strong>? This cannot
                            be undone.
                        </div>
                    )}
                </HFModalBody>
